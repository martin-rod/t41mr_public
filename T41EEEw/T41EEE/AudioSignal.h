...
AudioInputI2SQuad i2s_quadIn; // 4 inputs/outputs available only in Teensy audio
                              // and not Open Audio library.
...
AudioConvert_I16toF32
    int2Float1_tx; // Converts Int16 to Float.  See class in AudioStream_F32.h
AudioEffectGain_F32 micGain(audio_settings),
    compGainCompensate(audio_settings); // Microphone gain control.
...
AudioConvert_F32toI16 float2Int1_tx,
    float2Int2_tx; // Converts Float to Int16.  See class in AudioStream_F32.h
AudioSwitch4_OA_F32 switch1_tx, switch2_tx, switch3_tx, switch4_tx;
AudioMixer4_F32 mixer1_tx, mixer2_tx,
    mixer3_tx; // Used to switch in tone during calibration.
AudioSynthWaveformSine_F32 toneSSBCal1,
    toneSSBCal2;            // Tones for SSB calibration and IMD testing.
AudioRecordQueue Q_in_L_Ex; // AudioRecordQueue for input Microphone channel.
AudioRecordQueue
    Q_in_R_Ex; // This 2nd channel is needed as we are bringing I and Q into the
               // sketch instead of only microphone audio.
AudioPlayQueue
    Q_out_L_Ex; // AudioPlayQueue for driving the I channel (CW/SSB) to the QSE.
AudioPlayQueue
    Q_out_R_Ex; // AudioPlayQueue for driving the Q channel (CW/SSB) to the QSE.

//  Begin transmit signal chain.
AudioConnection connect0(i2s_quadIn, 0, int2Float1_tx,
                         0); // Microphone audio channel.  Must use int2Float
                             // because Open Audio does not have quad input.

AudioConnection_F32 connect1(int2Float1_tx, 0, switch1_tx,
                             0); // Used switches here because it appeared
                                 // necessary to stop flow of data.
AudioConnection_F32 connect2(toneSSBCal1, 0, switch2_tx,
                             0); // Tone used during SSB calibration.

// Need a mixer to switch in an audio tone during calibration.  Should be a
// nominal tone amplitude.
AudioConnection_F32
    connect3(switch1_tx, 0, mixer1_tx,
...
AudioEffectGain_F32 speakerVolume, speakerScale, headphoneVolume,
    headphoneScale, compGain;
...
AudioConnection
    patchCord2(i2s_quadIn, 3, ADC_RX_Q,
...
// Headphone path
AudioConnection_F32
    patchCord13(mixer4, 0, headphoneScale,
...
// used.
float32_t fBand1[] = {50.0,   70.711,   100.0,  141.421,  200.0,  282.843,
                      400.0,  565.685,  800.0,  1131.371, 1600.0, 2262.742,
                      3200.0, 4525.483, 6400.0, 24000.0};
// These are default values useful for SSB voice.  Bypass equalizer during FT8.
float32_t dbBand1[] = {-100.0, -100.0, -100.0, -100.0, -100.0, -100.0,
...
void initializeAudioPaths() {
  ...
  // <<<<<<<<<<<<<<<< basicCompressorBegin(pobject, linearInDB,
  // compressionRatio); The compression curve for basicCompressorBegin() has a 3
  // segments. It is linear up to an input linearInDB and then decreases gain
  // according to compressionRatioDB up to an input -10 dB where it is almost
...
void SetAudioOperatingState(RadioState operatingState) {
  ...
  Serial.printf("lastState=%d radioState=%d memory_used=%d memory_used_max=%d "
                ...
                lastState, radioState, (int)AudioStream::memory_used,
  ...
  case RadioState::CW_RECEIVE_STATE:
    ...
    if (ConfigData.AGCMode == true) { // Activate compressor2_1 path.
      ...
      mixer4.gain(0, 0.0);
      mixer4.gain(1, 1.0);
      compGain.setGain_dB(30.0); // Set the gain equal to the compressor2_1
                                 // nominal gain (below threshold).
    }
    ...
    speakerVolume.setGain(
        volumeLog[ConfigData.audioVolume]); // Set volume because sidetone may
                                            // have changed it.
    headphoneVolume.setGain(
        volumeLog[ConfigData.audioVolume]); // Set volume because sidetone may
                                            // have changed it.
    sgtl5000_1.volume(
        0.8); // Restore headphone volume after CW sidetone reduction.

    controlAudioOut(ConfigData.audioOut,
                    false); // Configure audio out; don't mute all.
  ...
  case RadioState::FT8_TRANSMIT_STATE:
    ...
    if (ConfigData.compressorFlag) {
      ...
      compGainCompensate.setGain_dB(
          10.0); // Use compressor's below threshold gain.
    }

    if (ConfigData.xmitEQFlag and
    ...
    Q_out_L_Ex.setBehaviour(
        AudioPlayQueue::ORIGINAL); // Need this as CW will put into wrong mode.
  ...
  case RadioState::SSB_CALIBRATE_STATE:
    ...
    Q_in_L_Ex.begin(); // I channel Microphone audio
    Q_in_R_Ex.begin(); // Q channel Microphone audio
    ADC_RX_I.begin(); // Calibration is full duplex!  Activate receiver data. No
                      // demodulation during calibrate, spectrum only.
    ADC_RX_Q.begin();
  ...
  case RadioState::SSB_IM3TEST_STATE:
    ...
    Q_in_L_Ex.begin(); // I channel Microphone audio
    Q_in_R_Ex.begin(); // Q channel Microphone audio
    //      ADC_RX_I.begin();   // Calibration is full duplex!  Activate
    //      receiver data.  No demodulation during calibrate, spectrum only.
    //      ADC_RX_Q.begin();
    patchCord25.disconnect();
    patchCord26.disconnect();
  ...
  case RadioState::CW_TRANSMIT_KEYER_STATE:
    ...
    sgtl5000_1.volume(static_cast<float32_t>(ConfigData.sidetoneHeadphone) /
                      100.0); // This is for scaling the sidetone in the
                              // headphone output.  This is the only way to
                              // adjust sidetone volume in the headphone.
    patchCord25.disconnect(); // Disconnect headphone, which is shared with I
                              // and Q transmit.
    patchCord26.disconnect();
    ...
    speakerVolume.setGain(
        volumeLog[ConfigData.sidetoneSpeaker]); // Adjust sidetone volume.
                                                // Headphone sidetone done in
                                                // hardware.
