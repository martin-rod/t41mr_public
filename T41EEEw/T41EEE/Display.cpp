...
// Analog Signal on this pin will be used for an analog S-Meter (250uA full scale) connected via 10kOhm to this output. 1uF
// capacitor paralle to the S-Meter. This might conflict with other hardware modifications, that might use Pin 33 for a different
// purpose, please check, before defining this
#if 0
#define HW_SMETER 33
#endif

// with TCVSDR_SMETER (TCVSDR=Teensy Convolution SDR) defined the S-Meter bar will be consistent with the dBm value and it will not
// go over the box for S9+40+
#define TCVSDR_SMETER 1

constexpr int AUDIO_SPECTRUM_TOP = 129;
constexpr int AUDIO_SPECTRUM_BOTTOM = SPECTRUM_BOTTOM;

constexpr int BAND_INDICATOR_X = WATERFALL_RIGHT_X + 25;
constexpr int BAND_INDICATOR_Y = WATERFALL_TOP_Y + 37; // 292

constexpr int TEMP_X_OFFSET = 635;
constexpr int TEMP_Y_OFFSET = 0;
constexpr int AGC_Y_OFFSET = 292;
constexpr int AGC_X_OFFSET = 680;

constexpr int X_R_STATUS_X = 730;
constexpr int X_R_STATUS_Y = 70;

constexpr int SMETER_BAR_HEIGHT = 18;
constexpr int SMETER_BAR_LENGTH = 180;

//  Offsets for status info
constexpr int FIELD_OFFSET_X = WATERFALL_RIGHT_X + 118; // X coordinate for field
constexpr int NOTCH_X = WATERFALL_RIGHT_X + 58;
constexpr int NOTCH_Y = WATERFALL_TOP_Y + 90;
constexpr int NOISE_REDUCE_X = WATERFALL_RIGHT_X + 58;
constexpr int NOISE_REDUCE_Y = WATERFALL_TOP_Y + 110;
constexpr int ZOOM_X = WATERFALL_RIGHT_X + 65;
constexpr int ZOOM_Y = WATERFALL_TOP_Y + 130;
constexpr int COMPRESSION_X = WATERFALL_RIGHT_X + 33;
constexpr int COMPRESSION_Y = WATERFALL_TOP_Y + 150;
constexpr int WPM_X = WATERFALL_RIGHT_X + 58;
constexpr int WPM_Y = WATERFALL_TOP_Y + 170;

const char RIGNAME[] = "T41-EP SDT";
constexpr int RIGNAME_X_OFFSET = 570; // Pixel count to rig name field

const uint16_t gradient[] = { // Color array for waterfall background
    0x0,    0x1,    0x2,    0x3,    0x4,    0x5,    0x6,    0x7,    0x8,    0x9,    0x10,   0x1F,   0x11F,  0x19F,  0x23F,
    0x2BF,  0x33F,  0x3BF,  0x43F,  0x4BF,  0x53F,  0x5BF,  0x63F,  0x6BF,  0x73F,  0x7FE,  0x7FA,  0x7F5,  0x7F0,  0x7EB,
    0x7E6,  0x7E2,  0x17E0, 0x3FE0, 0x67E0, 0x8FE0, 0xB7E0, 0xD7E0, 0xFFE0, 0xFFC0, 0xFF80, 0xFF20, 0xFEE0, 0xFE80, 0xFE40,
    0xFDE0, 0xFDA0, 0xFD40, 0xFD00, 0xFCA0, 0xFC60, 0xFC00, 0xFBC0, 0xFB60, 0xFB20, 0xFAC0, 0xFA80, 0xFA20, 0xF9E0, 0xF980,
    0xF940, 0xF8E0, 0xF8A0, 0xF840, 0xF800, 0xF802, 0xF804, 0xF806, 0xF808, 0xF80A, 0xF80C, 0xF80E, 0xF810, 0xF812, 0xF814,
    0xF816, 0xF818, 0xF81A, 0xF81C, 0xF81E, 0xF81E, 0xF81E, 0xF81E, 0xF83E, 0xF83E, 0xF83E, 0xF83E, 0xF85E, 0xF85E, 0xF85E,
    0xF85E, 0xF87E, 0xF87E, 0xF83E, 0xF83E, 0xF83E, 0xF83E, 0xF85E, 0xF85E, 0xF85E, 0xF85E, 0xF87E, 0xF87E, 0xF87E, 0xF87E,
    0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF88F, 0xF88F, 0xF88F};

const char *zoomOptions[] = {"1x ", "2x ", "4x ", "8x ", "16x"};
float32_t pixel_per_khz = 0.0;
int filterWidth = 0;
...
constexpr uint32_t SPLASH_DELAY = 3000L;

void tftPrintCenter(const char *text, int y) {
  tft.setCursor(CENTER, y);
  tft.print(text);
}

// The initial screen display on startup. Expect this to be customized
FLASHMEM void Splash() {
  int y = 0;
  tft.fillWindow(RA8875_BLACK);
  tft.setFontScale(2);
  tft.setTextColor(RA8875_GREEN);
  tftPrintCenter("T41-EP SDR Radio", y);
  y += tft.getFontHeight();
  tft.setFontScale(1);
  tft.setTextColor(RA8875_RED);
  tftPrintCenter("EXTREME EXPERIMENTER'S EDITION", y);
  y += tft.getFontHeight();
  tftPrintCenter(ConfigData.versionSettings, y);
  tft.setTextColor(RA8875_YELLOW);
  y += tft.getFontHeight();
  tftPrintCenter("By", y);
  y += tft.getFontHeight();
  tft.setTextColor(RA8875_WHITE);
  tftPrintCenter("Greg Raven KF5N", y);
  y += tft.getFontHeight();
  tft.setTextColor(RA8875_YELLOW);
  tftPrintCenter("Based on the T41-EP code by", y);
  tft.setTextColor(RA8875_WHITE);
  y += tft.getFontHeight();
  tftPrintCenter("Al Peter, AC8GY     Jack Purdum, W8TEE", y);
  y += tft.getFontHeight();
  tftPrintCenter("Property of:", y);
  tft.setTextColor(RA8875_GREEN);
  y += tft.getFontHeight();
  tftPrintCenter(ConfigData.myCallsign, y);
  y += tft.getFontHeight();
...
// Show the program name and version number.
void ShowName() {
  ...
  tft.setFontScale(0);
  tft.print(" ");
  if (ConfigData.hwVersion == HwVersion::T41_1_4SQRP) {
    tft.setTextColor(RA8875_GREEN);
  } else {
    tft.setTextColor(RA8875_RED);
  }
...
#define UPPERPIXTARGET 15
  ...
  pixelCurrent[1] = 0;
  //                512
  for (x1 = 1; x1 < MAX_WATERFALL_WIDTH - 1; x1++) // AFP, JJP changed init from 0 to 1 for x1: out of bounds
                                                   // addressing in line 112
  // Draws the main Spectrum, Waterfall and Audio displays
  {
    updateDisplayFlag = false;
    if ((ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_1) and static_cast<uint32_t>(ADC_RX_I.available()) > N_B and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_B) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1) {
        updateDisplayFlag = true;
      }
    }
    if ((ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_2) and static_cast<uint32_t>(ADC_RX_I.available()) > N_B and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_B) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1) {
        updateDisplayFlag = true;
    ...
    if ((ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_4) and static_cast<uint32_t>(ADC_RX_I.available()) > N_B and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_B) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1) {
        updateDisplayFlag = true;
    ...
    if ((ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_8) and static_cast<uint32_t>(ADC_RX_I.available()) > N_B and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_B) {
      updateDisplayCounter = updateDisplayCounter + 1;
    ...
    if ((ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_16) and static_cast<uint32_t>(ADC_RX_I.available()) > N_B and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_B) {
      updateDisplayCounter = updateDisplayCounter + 1;
    ...
    y_old = pixelold[x1]; // pixelold spectrum is saved by the FFT function prior to
    ...
    pixelCurrent[x1] = pixelnew[x1]; //  This is the actual "old" spectrum!  This is required
  ...
  if (keyPressedOn == 1) {
    ...
    tft.BTE_move(WATERFALL_LEFT_X, FIRST_WATERFALL_LINE, MAX_WATERFALL_WIDTH, MAX_WATERFALL_ROWS - 2, WATERFALL_LEFT_X,
    ...
    tft.BTE_move(WATERFALL_LEFT_X, FIRST_WATERFALL_LINE + 1, MAX_WATERFALL_WIDTH, MAX_WATERFALL_ROWS - 2, WATERFALL_LEFT_X,
    ...
    if (FH_max_box < LOWERPIXTARGET) { // LOWERPIXTARGET = 13
      if (ConfigData.autoGain) {
        ...
        if (ConfigData.rfGainCurrent > 25.0) {
          ConfigData.rfGainCurrent = 25.0; //  Do not allow RF gain greater than 25.
        }
      }
      if (ConfigData.autoSpectrum) {
        ConfigData.rfGainCurrent = ConfigData.rfGain[ConfigData.currentBand];
...
void ShowBandwidth() {
  ...
  MyDrawFloat(static_cast<float>(bands.bands[ConfigData.currentBand].FLoCut / 1000.0f), 2, 1, FILTER_PARAMETERS_X,
              FILTER_PARAMETERS_Y, buff);
  ...
  MyDrawFloat(static_cast<float>(bands.bands[ConfigData.currentBand].FHiCut / 1000.0f), 2, 1, FILTER_PARAMETERS_X + 80,
              FILTER_PARAMETERS_Y, buff);
  tft.print("kHz");
...
void DrawSMeterContainer() {
  ...
  tft.drawFastHLine(SMETER_X + 9 * pixels_per_s, SMETER_Y + SMETER_BAR_HEIGHT + 2, SMETER_BAR_LENGTH + 2 - 9 * pixels_per_s,
                    RA8875_GREEN);
  ...
  for (i = 1; i <= 3; i++) { // Draw tick marks for s9+ values in 10dB steps
    ...
    // DB2OO, 30-AUG-23: draw wider tick marks in the style of the Teensy
    // Convolution SDR
    tft.drawRect(SMETER_X + 9 * pixels_per_s + i * pixels_per_s * 10.0 / 6.0, SMETER_Y - 8 + (i % 2) * 2, 2, 8 - (i % 2) * 2,
                 RA8875_GREEN);
#else
  ...
  tft.setCursor(SMETER_X + 32,
                SMETER_Y - 25); // was 28, 48, 68, 88, 120 and -15 changed to -20
  tft.print("3");
...
void DrawSpectrumDisplayContainer() {
  if (calOnFlag) {
    ...
    tft.drawRect(SPECTRUM_LEFT_X - 1, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH + 2, 362,
                 RA8875_BLACK); // Erase spectrum box for calibration.
    tft.drawRect(SPECTRUM_LEFT_X - 1, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH + 2, SPECTRUM_HEIGHT,
...
void DrawFrequencyBarValue() {
  ...
  grat = static_cast<float>(SR[static_cast<size_t>(SampleRate)].rate / 8000.0) /
         static_cast<float>(1 << static_cast<int>(ConfigData.spectrum_zoom)); // 1, 2, 4, 8, 16, 32, 64 . . . 4096
  ...
  if (ConfigData.spectrum_zoom == SpectrumZoomState::SPECTRUM_ZOOM_1) {
    freq_calc += static_cast<float>(SR[static_cast<size_t>(SampleRate)].rate) / 4.0;
  }

  if (ConfigData.spectrum_zoom <= SpectrumZoomState::SPECTRUM_ZOOM_16) {
  ...
  **************************************************************************************************/
  ...
  for (int idx = -4; idx < 5; idx++) {
    pos_help = idx2pos[ConfigData.spectrum_zoom < SpectrumZoomState::SPECTRUM_ZOOM_8 ? 0 : 1][idx + 4];
    if (idx != centerIdx) {
      ...
      tft.print(txt);
      if (idx < 4) {
        tft.drawFastVLine((WATERFALL_LEFT_X + pos_help * xExpand + 60), WATERFALL_TOP_Y - 5, 7,
    ...
    if (ConfigData.spectrum_zoom > SpectrumZoomState::SPECTRUM_ZOOM_4 || freq_calc > 1000) {
      idx++;
    }
  }
...
void ShowVFOStatus() {
  ...
  tft.fillRect(OPERATION_STATS_X + 275, FREQUENCY_Y + 30, tft.getFontWidth() * 6, tft.getFontHeight(), RA8875_BLACK);
...
void BandInformation() // SSB or CW
{
  ...
  // Clear frequency, band, and mode.  This should be the only erase required.
  tft.fillRect(5, FREQUENCY_Y + 31, 290, 15, RA8875_BLACK);
...
void ShowCurrentPowerSetting() {
  tft.setFontScale((enum RA8875tsize)0);
  // Clear top-left menu area
  tft.fillRect(OPERATION_STATS_X + 275 + 75, FREQUENCY_Y + 30, tft.getFontWidth() * 5, tft.getFontHeight(), RA8875_BLACK);
  tft.setCursor(OPERATION_STATS_X + 275 + 75, FREQUENCY_Y + 30);
  tft.setTextColor(RA8875_RED);
  tft.printf("%02.0f W", ConfigData.transmitPowerLevel);
  tft.setTextColor(RA8875_WHITE);
}
...
void FormatFrequency(uint32_t freq, char *freqBuffer, size_t freqBufferSize) {
  ...
  case 6: // below 530.999 KHz
    ...
    freqBuffer[3] = DEFAULT_FREQ_SEP_CHARACTER; // Add separation charcter
    for (i = 4; i < len; i++) {
  ...
  case 7: // 1.0 - 9.999 MHz
    freqBuffer[0] = outBuffer[0];
    freqBuffer[1] = DEFAULT_FREQ_SEP_CHARACTER; // Add separation charcter
    for (i = 2; i < 5; i++) {
      freqBuffer[i] = outBuffer[i - 1]; // Next 3 digit chars
    }
    freqBuffer[5] = DEFAULT_FREQ_SEP_CHARACTER; // Add separation charcter
    for (i = 6; i < 9; i++) {
  ...
  case 8: // 10 MHz - 30MHz
    freqBuffer[0] = outBuffer[0];
    freqBuffer[1] = outBuffer[1];
    freqBuffer[2] = DEFAULT_FREQ_SEP_CHARACTER; // Add separation charcter
    for (i = 3; i < 6; i++) {
      freqBuffer[i] = outBuffer[i - 1]; // Next 3 digit chars
    }
    freqBuffer[6] = DEFAULT_FREQ_SEP_CHARACTER; // Add separation charcter
    for (i = 7; i < 10; i++) {
      freqBuffer[i] = outBuffer[i - 2]; // Last 3 digit chars
    }
    freqBuffer[i] = '\0'; // Make it a string
...
void ShowFrequency() {
  ...
  case VfoState::VFO_B: // Show VFO_B
    ...
    tft.fillRect(FREQUENCY_X_SPLIT - 60, FREQUENCY_Y - 14, tft.getFontWidth() * 10, tft.getFontHeight(),
                 RA8875_BLACK); // JJP 7/15/23
...
void DisplaydbM() {
  ...
  dbm = CalData.dBm_calibration + bands.bands[ConfigData.currentBand].gainCorrection + static_cast<float32_t>(attenuator) +
        slope * log10f_fast(audioMaxSquaredAve) + cons - static_cast<float32_t>(bands.bands[ConfigData.currentBand].RFgain) * 1.5 -
...
//   SMETER_BAR_HEIGHT, RA8875_RED);  //AFP 09-18-22
  ...
  // DB2OO, 13-OCT-23: if DEBUG_SMETER defined debug messages on S-Metervariables will be put out
#ifdef DEBUG_SMETER
  Serial.printf("DisplaydbM(): dbm=%.1f, dbm_calibration=%.1f, "
                ...
                dbm, dbm_calibration, bands.bands[ConfigData.currentBand].gainCorrection, attenuator,
                bands.bands[ConfigData.currentBand].RFgain, ConfigData.rfGainAllBands);
  ...
  MyDrawFloat(dbm, 4, 1, SMETER_X + 184, SMETER_Y, buff);
  tft.setTextColor(RA8875_GREEN);
...
void ShowTempAndLoad() {
  ...
  double elapsed_micros_mean = (double)elapsed_micros_sum / (double)elapsed_micros_idx_t;

  block_time = 128.0 / (double)SR[static_cast<size_t>(SampleRate)].rate; // one audio block is 128 samples and uses this in seconds
  block_time = block_time * N_B;
  ...
  tft.setTextColor(RA8875_WHITE);
  tft.print("Temp:");
  tft.setCursor(TEMP_X_OFFSET + 90, TEMP_Y_OFFSET);
  tft.print("Load:");

  tft.setTextColor(valueColor);
  MyDrawFloat(CPU_temperature, 3, 0, TEMP_X_OFFSET + tft.getFontWidth() * 5, TEMP_Y_OFFSET, buff);
  tft.print("C");
  MyDrawFloat(processor_load, 2, 0, TEMP_X_OFFSET + 90 + tft.getFontWidth() * 5, TEMP_Y_OFFSET, buff);
  tft.print("%");
...
void MyDrawFloat(float val, int width, int precision, int x, int y, char *buff) {
  dtostrf(val, width, precision, buff);
  size_t charCnt = strlen(buff);
  tft.fillRect(x, y, 12 * charCnt, 15, RA8875_BLACK);
  tft.setCursor(x, y);
  tft.print(buff);
}
...
// Updates the zoom setting on the display
...
// Updates the compression setting in the info window.
void UpdateCompressionField() {
  ...
  if (ConfigData.compressorFlag == true) {
    tft.print("On  ");
    // This is the compression ratio of the Open Audio compressor.
...
// Updates whether the decoder is on or off and decoder related graphics including CW, highpass, and lowpass filter bandwidths.
FLASHMEM void UpdateAudioGraphics() {
  ...
  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER or
    ...
    if (not switchFilterSideband) {
      tft.drawLine(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker), SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker), SPECTRUM_BOTTOM - 112, RA8875_LIGHT_GREY);
      tft.drawLine(BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker), SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker), SPECTRUM_BOTTOM - 112, RA8875_RED);
    } else {
      tft.drawLine(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker), SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker), SPECTRUM_BOTTOM - 112, RA8875_RED);
      tft.drawLine(BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker), SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker), SPECTRUM_BOTTOM - 112, RA8875_LIGHT_GREY);
    }
  }
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and ConfigData.CWFilterIndex != 5) {
    // This has to be written to L2 or it will be erased by the audio spectrum eraser.
    tft.writeTo(L2);
    tft.fillRect(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker), AUDIO_SPECTRUM_TOP,
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and ConfigData.decoderFlag) {
    ...
    if (ConfigData.CWOffset == 0) {
      // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 15, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
      // CW upper freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 21, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
    }
    if (ConfigData.CWOffset == 1) {
      // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 18, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
      // CW upper freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 24, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
    }
    if (ConfigData.CWOffset == 2) {
      // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 23, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
      // CW upper freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 29, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
    }
    if (ConfigData.CWOffset == 3) {
      // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 26, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
      // CW upper freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 32, AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_GREEN);
    }
  }
...
void UpdateWPMField() {
  ...
  if (ConfigData.keyType == KeyTypeEnum::PADDLES) {
    tft.print("Paddles ");
    if (ConfigData.paddleFlip == PaddleFlipEnum::RIGHT_PADDLE_DAH) {
      tft.print("R");
    } else {
      tft.print("L");
    }
...
void DrawBandWidthIndicatorBar() // AFP 10-30-22
{
  ...
  newCursorPosition =
      static_cast<int>(NCOFreq / hz_per_pixel) + Zoom1Offset; // More accurate tuning bar position.  KF5N May 17, 2024
  ...
  pixel_per_khz =
      ((1 << static_cast<int>(ConfigData.spectrum_zoom)) * SPECTRUM_RES * 1000.0 / SR[static_cast<size_t>(SampleRate)].rate);
  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER or
      bands.bands[ConfigData.currentBand].sideband == Sideband::UPPER) {
    filterWidth =
        static_cast<int>(((bands.bands[ConfigData.currentBand].FHiCut - bands.bands[ConfigData.currentBand].FLoCut) / 1000.0) *
                         pixel_per_khz * 1.06);
  } else if (bands.bands[ConfigData.currentBand].sideband == Sideband::BOTH_AM or
             bands.bands[ConfigData.currentBand].sideband == Sideband::BOTH_SAM) {
    filterWidth = static_cast<int>(((bands.bands[ConfigData.currentBand].FAMCut * 2.0) / 1000.0) * pixel_per_khz * 1.06);
  }

  switch (bands.bands[ConfigData.currentBand].sideband) {
  case Sideband::LOWER:
    tft.fillRect(centerLine - filterWidth + oldCursorPosition, SPECTRUM_TOP_Y + 20, filterWidth * 1.0, SPECTRUM_HEIGHT - 20,
                 RA8875_BLACK);
  ...
  case Sideband::BOTH_AM:
  case Sideband::BOTH_SAM:
    tft.fillRect(centerLine - filterWidth / 2 + oldCursorPosition, SPECTRUM_TOP_Y + 20, filterWidth, SPECTRUM_HEIGHT - 20,
                 RA8875_BLACK); // AFP 10-30-22
    tft.fillRect(centerLine - filterWidth / 2 * 0.93 + newCursorPosition, SPECTRUM_TOP_Y + 20, filterWidth * 0.95,
                 SPECTRUM_HEIGHT - 20,
...
void ShowTransmitReceiveStatus() {
  ...
  if (radioState == RadioState::SSB_TRANSMIT_STATE or radioState == RadioState::FT8_TRANSMIT_STATE or
      radioState == RadioState::CW_TRANSMIT_STRAIGHT_STATE or radioState == RadioState::CW_TRANSMIT_KEYER_STATE or
      radioState == RadioState::CW_CALIBRATE_STATE or radioState == RadioState::SSB_CALIBRATE_STATE) {
