...
#define CLIP_AUDIO_PEAK                                                        \
  115 // The pixel value where audio peak overwrites S-meter
...
const uint16_t gradient[] = { // Color array for waterfall background
    ...
    0x9,    0x10,   0x1F,   0x11F,  0x19F,  0x23F,  0x2BF,  0x33F,  0x3BF,
    0x43F,  0x4BF,  0x53F,  0x5BF,  0x63F,  0x6BF,  0x73F,  0x7FE,  0x7FA,
    0x7F5,  0x7F0,  0x7EB,  0x7E6,  0x7E2,  0x17E0, 0x3FE0, 0x67E0, 0x8FE0,
    0xB7E0, 0xD7E0, 0xFFE0, 0xFFC0, 0xFF80, 0xFF20, 0xFEE0, 0xFE80, 0xFE40,
    0xFDE0, 0xFDA0, 0xFD40, 0xFD00, 0xFCA0, 0xFC60, 0xFC00, 0xFBC0, 0xFB60,
    0xFB20, 0xFAC0, 0xFA80, 0xFA20, 0xF9E0, 0xF980, 0xF940, 0xF8E0, 0xF8A0,
    0xF840, 0xF800, 0xF802, 0xF804, 0xF806, 0xF808, 0xF80A, 0xF80C, 0xF80E,
    0xF810, 0xF812, 0xF814, 0xF816, 0xF818, 0xF81A, 0xF81C, 0xF81E, 0xF81E,
    0xF81E, 0xF81E, 0xF83E, 0xF83E, 0xF83E, 0xF83E, 0xF85E, 0xF85E, 0xF85E,
    0xF85E, 0xF87E, 0xF87E, 0xF83E, 0xF83E, 0xF83E, 0xF83E, 0xF85E, 0xF85E,
    0xF85E, 0xF85E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E,
    0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF87E, 0xF88F, 0xF88F, 0xF88F};
...
void DrawAudioSpectContainer() {
  tft.writeTo(L1);
  tft.drawRect(BAND_INDICATOR_X - 9, SPECTRUM_BOTTOM - 118, 255, 118,
               RA8875_GREEN);
  tft.setFontScale((enum RA8875tsize)0);
  tft.setTextColor(RA8875_WHITE);
  for (int k = 0; k < 6; k++) {
    tft.drawFastVLine(BAND_INDICATOR_X - 10 + k * 43.8, SPECTRUM_BOTTOM, 15,
                      RA8875_GREEN);
...
*****/
void ShowName() {
  tft.fillRect(RIGNAME_X_OFFSET, 0, XPIXELS - RIGNAME_X_OFFSET,
               tft.getFontHeight(), RA8875_BLACK);
...
#ifdef FOURSQRP // Give visual indication that were using the 4SQRP code.  W8TEE
                // October 11, 2023.
...
#define UPPERPIXTARGET 15
  ...
  char buff[10];
  int frequ_hist[32]{
      0}; // All values are initialized to zero using this syntax.
  int audio_hist[256]{
      0}; // All values are initialized to zero using this syntax.
  int j, k;
  ...
  tft.drawFastVLine(centerLine, SPECTRUM_TOP_Y, h,
                    RA8875_GREEN); // Draws centerline on spectrum display
  ...
  pixelCurrent[1] = 0;
  //                512
  for (x1 = 1; x1 < MAX_WATERFALL_WIDTH - 1;
       x1++) // AFP, JJP changed init from 0 to 1 for x1: out of bounds
             // addressing in line 112
  // Draws the main Spectrum, Waterfall and Audio displays
  {
    updateDisplayFlag = false;
    if ((ConfigData.spectrum_zoom == 0) and
        static_cast<uint32_t>(ADC_RX_I.available()) > N_BLOCKS and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_BLOCKS) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1)
        updateDisplayFlag = true;
    }
    if ((ConfigData.spectrum_zoom == 1) and
        static_cast<uint32_t>(ADC_RX_I.available()) > N_BLOCKS and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_BLOCKS) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1)
    ...
    if ((ConfigData.spectrum_zoom == 2) and
        static_cast<uint32_t>(ADC_RX_I.available()) > N_BLOCKS and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_BLOCKS) {
      updateDisplayCounter = updateDisplayCounter + 1;
      if (updateDisplayCounter == 1)
    ...
    if ((ConfigData.spectrum_zoom == 3) and
        static_cast<uint32_t>(ADC_RX_I.available()) > N_BLOCKS and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_BLOCKS) {
      updateDisplayCounter = updateDisplayCounter + 1;
    ...
    if ((ConfigData.spectrum_zoom == 4) and
        static_cast<uint32_t>(ADC_RX_I.available()) > N_BLOCKS and
        static_cast<uint32_t>(ADC_RX_Q.available()) > N_BLOCKS) {
      updateDisplayCounter = updateDisplayCounter + 1;
    ...
    // This requires 2 global variables.
    if (filter_pos != last_filter_pos)
      FilterSetSSB();

    process.ProcessIQData(); // Call the Audio process from within the display
                             // routine to eliminate conflicts with drawing the
                             // spectrum and waterfall displays

    EncoderCenterTune(); // Moved the tuning encoder to reduce lag times and
                         // interference during tuning.
    y_new = pixelnew[x1];
    y1_new = pixelnew[x1 - 1];
    y_old =
        pixelold[x1]; // pixelold spectrum is saved by the FFT function prior to
                      // a new FFT which generates the pixelnew spectrum.  KF5N
    ...
    // display top.
    if ((x1 > 51) &&
        (x1 < 461)) //  HB start for auto RFgain collect frequency distribution.
                    //  Limited to core of FFT and dividable by 4.
    {
      j = 247 - y_new_plot +
          40; // +40 to get 10 bins below zero - want to straddle zero to make
              // the entire spectrum viewable.
      k = j >> 2;                 // Divide by 4
      if ((k > -1) && (k < 32)) { // The index of the bin array.  This indicates
                                  // a value within the defined spectral box.
    ...
    // Collect a histogram of audio spectral values.  This is used to keep the
    // audio spectrum in the viewable area. 247??? is the spectral display
    // bottom.  129 is the audio spectrum display top.
    if ((x1 < 256) and
        (audioYPixel[x1] >
         0)) //  Collect audio frequency distribution to find noise floor.
    {
      k = audioYPixel[x1]; // +40 to get 10 bins below zero - want to straddle
                           // zero to make the entire spectrum viewable.
      audio_hist[k] +=
          1; // Add (accumulate) to the bin.
             //       if(x1 == 50) Serial.printf("audio_hist[k] = %d AudioH_max
      ...
      if (audio_hist[k] > AudioH_max) { // FH_max starts at 0.
        ...
        AudioH_max_box =
            k; // Index of FH_max.  this corresponds to the noise floor.
    ...
    tft.drawLine(x1 + 1, y_old2_plot, x1 + 1, y_old_plot,
                 RA8875_BLACK); // Erase old...
    tft.drawLine(x1 + 1, y1_new_plot, x1 + 1, y_new_plot,
                 RA8875_YELLOW); // Draw new

    //  What is the actual spectrum at this time?  It's a combination of the old
    //  and new spectrums. In the case of a CW interrupt, the array pixelnew
    //  should be saved as the actual spectrum.
    pixelCurrent[x1] =
        pixelnew[x1]; //  This is the actual "old" spectrum!  This is required
                      //  due to CW interrupts.  pixelCurrent gets copied to
    ...
    if (x1 < 253) {            // AFP 09-01-22
      if (keyPressedOn == 1) { // AFP 09-01-22
        ...
        if (audioYPixelold[x1] > CLIP_AUDIO_PEAK)
          audioYPixelold[x1] = CLIP_AUDIO_PEAK; // audioSpectrumHeight = 118
        tft.drawFastVLine(532 + x1, 245 - audioYPixelold[x1] - 0,
                          audioYPixelold[x1], RA8875_BLACK);
        if (audioYPixel[x1] != 0) {
          ...
          // Draw a vertical line with the audio spectrum magnitude.
          // AUDIO_SPECTRUM_BOTTOM = 247
          tft.drawFastVLine(532 + x1, 245 - audioYPixel[x1] - 0,
                            audioYPixel[x1], RA8875_MAGENTA);
        }
      }
    }

    test1 =
        -y_new_plot + 230; // Nudged waterfall towards blue.  KF5N July 23, 2023
    if (test1 < 0)
      test1 = 0;
    if (test1 > 117)
      test1 = 117;
    waterfall[x1] = gradient[test1]; // Try to put pixel values in middle of
                                     // gradient array.  KF5N
    tft.writeTo(L1);
  ...
  if (keyPressedOn == 1) {
    ...
    tft.BTE_move(WATERFALL_LEFT_X, FIRST_WATERFALL_LINE, MAX_WATERFALL_WIDTH,
                 MAX_WATERFALL_ROWS - 2, WATERFALL_LEFT_X,
                 FIRST_WATERFALL_LINE + 1, 1, 2);
    ...
    tft.BTE_move(WATERFALL_LEFT_X, FIRST_WATERFALL_LINE + 1,
                 MAX_WATERFALL_WIDTH, MAX_WATERFALL_ROWS - 2, WATERFALL_LEFT_X,
                 FIRST_WATERFALL_LINE + 1, 2);
    while (tft.readStatus())
      ; // Make sure it's done.
  }
  // Then write new row data into the missing top row to get a scroll effect
  // using display hardware, not the CPU.
  tft.writeRect(WATERFALL_LEFT_X, FIRST_WATERFALL_LINE, MAX_WATERFALL_WIDTH, 1,
                waterfall);

  // Manage RF spectral display graphics.  Keep the spectrum within the viewable
  // area.
  if (ConfigData.autoGain || ConfigData.autoSpectrum) {
    if (FH_max_box >
        UPPERPIXTARGET) { // HB. Adjust rfGainAllBands 15 and 13 to alter to
                          // move target base up and down. UPPERPIXTARGET = 15
    ...
    if (FH_max_box < LOWERPIXTARGET) { // LOWERPIXTARGET = 13
      if (ConfigData.autoGain) {
        ConfigData.rfGainCurrent = ConfigData.rfGainCurrent + 1;
        if (ConfigData.rfGainCurrent > 25.0)
          ConfigData.rfGainCurrent =
              25.0; //  Do not allow RF gain greater than 25.
      }
      if (ConfigData.autoSpectrum) {
        ConfigData.rfGainCurrent = ConfigData.rfGain[ConfigData.currentBand];
        fftOffset = fftOffset + 1;
  ...
  // Manage audio spectral display graphics.  Keep the spectrum within the
  // viewable area.

  if (AudioH_max_box >
      30) { // HB. Adjust rfGainAllBands 15 and 13 to alter to move target base
            // up and down. UPPERPIXTARGET = 15
  ...
  tft.fillRect(SPECTRUM_LEFT_X + 125, SPECTRUM_TOP_Y + 2, 33,
               tft.getFontHeight(), RA8875_BLACK);
  tft.setFontScale((enum RA8875tsize)0);
  tft.setTextColor(RA8875_WHITE);
  ...
  if (ConfigData.autoGain)
    itoa(ConfigData.rfGainCurrent, buff, DEC); // Make into a string
  else
    itoa(ConfigData.rfGain[ConfigData.currentBand], buff, DEC);
...
void ShowBandwidth() {
  ...
  pos_left =
      centerLine + static_cast<int>(bands.bands[ConfigData.currentBand].FLoCut /
                                    1000.0 * pixel_per_khz);
  ...
  MyDrawFloat(
      static_cast<float>(bands.bands[ConfigData.currentBand].FLoCut / 1000.0f),
      1, FILTER_PARAMETERS_X, FILTER_PARAMETERS_Y, buff);
  ...
  if (switchFilterSideband == true)
    ...
    tft.setTextColor(RA8875_LIGHT_GREY);
  MyDrawFloat(
      static_cast<float>(bands.bands[ConfigData.currentBand].FHiCut / 1000.0f),
      1, FILTER_PARAMETERS_X + 80, FILTER_PARAMETERS_Y, buff);
  tft.print("kHz");

  tft.setTextColor(RA8875_WHITE); // set text color to white for other print
                                  // routines not to get confused ;-)
  tft.writeTo(L1);
}

// DB2OO, 30-AUG-23: this variable determines the pixels per S step. In the
// original code it was 12.2 pixels !?
#ifdef TCVSDR_SMETER
...
void DrawSMeterContainer() {
  ...
  tft.drawFastHLine(SMETER_X, SMETER_Y + SMETER_BAR_HEIGHT + 2,
                    9 * pixels_per_s, RA8875_WHITE); // changed 6 to 20

  for (i = 0; i < 10; i++) { // Draw tick marks for S-values
#ifdef TCVSDR_SMETER
    // DB2OO, 30-AUG-23: draw wider tick marks in the style of the Teensy
    // Convolution SDR
    tft.drawRect(SMETER_X + i * pixels_per_s, SMETER_Y - 6 - (i % 2) * 2, 2,
                 6 + (i % 2) * 2, RA8875_WHITE);
#else
  ...
  tft.drawFastHLine(SMETER_X + 9 * pixels_per_s, SMETER_Y - 1,
                    SMETER_BAR_LENGTH + 2 - 9 * pixels_per_s, RA8875_GREEN);
  tft.drawFastHLine(SMETER_X + 9 * pixels_per_s,
                    SMETER_Y + SMETER_BAR_HEIGHT + 2,
                    SMETER_BAR_LENGTH + 2 - 9 * pixels_per_s, RA8875_GREEN);
  ...
  for (i = 1; i <= 3; i++) { // Draw tick marks for s9+ values in 10dB steps
    ...
    // DB2OO, 30-AUG-23: draw wider tick marks in the style of the Teensy
    // Convolution SDR
    tft.drawRect(SMETER_X + 9 * pixels_per_s + i * pixels_per_s * 10.0 / 6.0,
                 SMETER_Y - 8 + (i % 2) * 2, 2, 8 - (i % 2) * 2, RA8875_GREEN);
#else
    tft.drawFastVLine(SMETER_X + 9 * pixels_per_s +
                          i * pixels_per_s * 10.0 / 6.0,
                      SMETER_Y - 6, 7, RA8875_GREEN);
#endif
  }

  tft.drawFastVLine(SMETER_X, SMETER_Y - 1, SMETER_BAR_HEIGHT + 3,
                    RA8875_WHITE);
  tft.drawFastVLine(SMETER_X + SMETER_BAR_LENGTH + 2, SMETER_Y - 1,
                    SMETER_BAR_HEIGHT + 3, RA8875_GREEN);
  ...
  tft.setCursor(SMETER_X + 8, SMETER_Y - 25);
  tft.print("1");
  tft.setCursor(SMETER_X + 32,
                SMETER_Y -
                    25); // was 28, 48, 68, 88, 120 and -15 changed to -20
  tft.print("3");
...
void ShowSpectrumdBScale() {
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(SPECTRUM_LEFT_X + 1, SPECTRUM_TOP_Y + 2, 33, tft.getFontHeight(),
               RA8875_BLACK);
...
void DrawSpectrumDisplayContainer() {
  if (calOnFlag)
    tft.drawRect(SPECTRUM_LEFT_X - 1, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH + 2,
                 362, RA8875_YELLOW); // Spectrum box for calibration.
  else {
    tft.drawRect(SPECTRUM_LEFT_X - 1, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH + 2,
                 362, RA8875_BLACK); // Erase spectrum box for calibration.
    tft.drawRect(SPECTRUM_LEFT_X - 1, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH + 2,
                 SPECTRUM_HEIGHT,
                 RA8875_YELLOW); // Spectrum box.  SPECTRUM_HEIGHT = 150
  }
}
...
void DrawFrequencyBarValue() {
  ...
  // positions for graticules: first for ConfigData.spectrum_zoom < 3, then for
  // ConfigData.spectrum_zoom > 2
  ...
  grat =
      static_cast<float>(SR[SampleRate].rate / 8000.0) /
      static_cast<float>(
          1 << ConfigData.spectrum_zoom); // 1, 2, 4, 8, 16, 32, 64 . . . 4096

  tft.writeTo(L2); // Not writing to correct layer?  KF5N.  July 31, 2023
  tft.setTextColor(RA8875_WHITE);
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(WATERFALL_LEFT_X, WATERFALL_TOP_Y, MAX_WATERFALL_WIDTH + 5,
               tft.getFontHeight(), RA8875_BLACK); // 4-16-2022 JACK

  freq_calc = static_cast<float>(static_cast<uint32_t>(
      ConfigData.centerFreq)); // get current frequency in Hz
  ...
  if (ConfigData.spectrum_zoom < 5) {
    freq_calc = roundf(freq_calc / 100) /
                10; // round graticule frequency to the nearest 100Hz
  }
  ...
  **************************************************************************************************/
  ...
  for (int idx = -4; idx < 5; idx++) {
    ...
    if (idx != centerIdx) {
      ...
      //================== AFP 10-21-22 =============
      if (ConfigData.spectrum_zoom == 0) {
        tft.setCursor(WATERFALL_LEFT_X + pos_help * xExpand + 40,
                      WATERFALL_TOP_Y); // AFP 10-20-22
      } else {
        tft.setCursor(WATERFALL_LEFT_X + pos_help * xExpand + 40,
                      WATERFALL_TOP_Y); // AFP 10-20-22
      }
      ...
      tft.print(txt);
      if (idx < 4) {
        tft.drawFastVLine((WATERFALL_LEFT_X + pos_help * xExpand + 60),
                          WATERFALL_TOP_Y - 5, 7,
                          RA8875_YELLOW); // Tick marks depending on zoom
      } else {
        tft.drawFastVLine((WATERFALL_LEFT_X + (pos_help + 9) * xExpand + 60),
                          WATERFALL_TOP_Y - 5, 7, RA8875_YELLOW);
      }
    }
...
void ShowAutoStatus() {
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(SPECTRUM_LEFT_X + 350, SPECTRUM_TOP_Y + 2, 130,
               tft.getFontHeight(), RA8875_BLACK);
...
{
  std::string CWFilter[] = {"0.8kHz", "1.0kHz", "1.3kHz",
                            "1.8kHz", "2.0kHz", " Off "};
  ...
  tft.fillRect(100, FREQUENCY_Y + 31, 290, 15,
               RA8875_BLACK); // Clear frequency, band, and mode.  This should
                              // be the only erase required.
  ...
  if (ConfigData.activeVFO == VFO_A) {
    tft.print(bands.bands[ConfigData.currentBandA]
                  .name); // Write current band to the display.
  } else {
...
void ShowCurrentPowerSetting() {
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(OPERATION_STATS_X + 275, FREQUENCY_Y + 30,
               tft.getFontWidth() * 11, tft.getFontHeight(),
               RA8875_BLACK); // Clear top-left menu area
...
void ShowFrequency() {
  ...
  if (ConfigData.activeVFO == VFO_A) {
    ...
    if (TxRxFreq < bands.bands[ConfigData.currentBandA].fBandLow or
        TxRxFreq > bands.bands[ConfigData.currentBandA].fBandHigh) {
      tft.setTextColor(RA8875_RED); // Out of band
    } else {
      tft.setTextColor(RA8875_GREEN); // In band
    }
    tft.fillRect(0, FREQUENCY_Y - 8, tft.getFontWidth() * 10, 34,
                 RA8875_BLACK); // This erases VFOA.
    tft.setCursor(0, FREQUENCY_Y -
                         17); // To adjust for Greg's font change jjp 7/14/23
    ...
    tft.setFontScale(3, 2); // JJP 7/15/23
                            ...
                            //  RA8875_BLACK);  //JJP 7/15/23
    tft.fillRect(FREQUENCY_X_SPLIT - 60, FREQUENCY_Y - 14,
                 tft.getFontWidth() * 10, tft.getFontHeight(),
                 RA8875_BLACK); // JJP 7/15/23
    tft.setCursor(FREQUENCY_X_SPLIT - 60, FREQUENCY_Y - 12);
    if (TxRxFreq < bands.bands[ConfigData.currentBandB].fBandLow ||
        TxRxFreq > bands.bands[ConfigData.currentBandB].fBandHigh) {
      tft.setTextColor(RA8875_RED);
    } else {
      tft.setTextColor(RA8875_GREEN);
    }
    ...
    tft.setFontScale(1, 2); // JJP 7/15/23
    FormatFrequency(TxRxFreq, freqBuffer);
    tft.fillRect(0, FREQUENCY_Y - 14, tft.getFontWidth() * 10,
                 tft.getFontHeight(), RA8875_BLACK); // JJP 7/15/23
    tft.setTextColor(RA8875_LIGHT_GREY);
...
void DisplaydbM() {
  ...
  // DB2OO, 30-AUG-23: the S-Meter bar and the dBm value were inconsistent, as
  // they were using different base values.
  //  Moreover the bar could go over the limits of the S-meter box, as the map()
  //  function, does not constrain the values with TCVSDR_SMETER defined the
  //  S-Meter bar will be consistent with the dBm value and the S-Meter bar will
  //  always be restricted to the box
  tft.fillRect(SMETER_X + 1, SMETER_Y + 1, SMETER_BAR_LENGTH, SMETER_BAR_HEIGHT,
               RA8875_BLACK); // AFP 09-18-22  Erase old bar
#ifdef TCVSDR_SMETER
  // DB2OO, 9-OCT_23: dbm_calibration set to -22 in SDT.ino; gainCorrection is a
  // value between -2 and +6 to compensate the frequency dependant pre-Amp gain
  //  attenuator is 0 and could be set in a future HW revision; RFgain is
  //  initialized to 1 in the bands.bands[] init in SDT.ino; cons=-92; slope=10
  if (ConfigData.autoGain)
    rfGain = ConfigData.rfGainCurrent;
  else
    rfGain = ConfigData.rfGain[ConfigData.currentBand];
  dbm =
      CalData.dBm_calibration +
      bands.bands[ConfigData.currentBand].gainCorrection +
      static_cast<float32_t>(attenuator) +
      slope * log10f_fast(audioMaxSquaredAve) + cons -
      static_cast<float32_t>(bands.bands[ConfigData.currentBand].RFgain) * 1.5 -
      rfGain; // DB2OO, 08-OCT-23; added ConfigData.rfGainAllBands
#else
  // DB2OO, 9-OCT-23: audioMaxSquaredAve is proportional to the input power.
  // With ConfigData.rfGainAllBands=0 it is approx. 40 for -73dBm @ 14074kHz
  // with the V010 boards and the pre-Amp fed by 12V
  //  for audioMaxSquaredAve=40 audioLogAveSq will be 26
  audioLogAveSq = 10 * log10f_fast(audioMaxSquaredAve) + 10; // AFP 09-18-22
  // DB2OO, 9-OCT-23: calculate dBm value from audioLogAveSq and ignore band
  // gain differences and a potential attenuator like in original code
  dbm = audioLogAveSq - 100;

// DB2OO, 9-OCT-23: this is the orginal code, that will map a 30dB difference
// (35-5) to 60 (635-575) pixels, i.e. 5 S steps to 5*12 pixels
//  SMETER_X is 528 --> X=635 would be 107 pixels / 12pixels per S step -->
//  approx. S9
//   smeterPad = map(audioLogAveSq, 5, 35, 575, 635); //AFP 09-18-22
//   tft.fillRect(SMETER_X + 1, SMETER_Y + 1, smeterPad - SMETER_X,
//   SMETER_BAR_HEIGHT, RA8875_RED);  //AFP 09-18-22
#endif
  // determine length of S-meter bar, limit it to the box and draw it
  smeterPad =
      map(dbm, -73.0 - 9 * 6.0 /*S1*/, -73.0 /*S9*/, 0, 9 * pixels_per_s);
  ...
  tft.fillRect(SMETER_X + 1, SMETER_Y + 2, smeterPad, SMETER_BAR_HEIGHT - 2,
               RA8875_RED); // DB2OO: bar 2*1 pixel smaller than the field

  tft.setTextColor(RA8875_WHITE);

  // DB2OO, 17-AUG-23: create PWM analog output signal on the "HW_SMETER"
  // output. This is scaled for a 250uA  S-meter full scale,
  //  connected to HW_SMTER output via a 8.2kOhm resistor and a 4.7kOhm resistor
  //  and 10uF capacitor parallel to the S-Meter
#ifdef HW_SMETER
  {
    int hw_s;
  ...
  // DB2OO, 13-OCT-23: if DEBUG_SMETER defined debug messages on S-Meter
  // variables will be put out
  // #define DEBUG_SMETER

#ifdef DEBUG_SMETER
  // added, to debug S-Meter display problems
  Serial.printf("DisplaydbM(): dbm=%.1f, dbm_calibration=%.1f, "
                "bands.bands[ConfigData.currentBand].gainCorrection=%.1f, "
                "attenuator=%d, bands.bands[ConfigData.currentBand].RFgain=%d, "
                "ConfigData.rfGainAllBands=%d\n",
                dbm, dbm_calibration,
                bands.bands[ConfigData.currentBand].gainCorrection, attenuator,
                bands.bands[ConfigData.currentBand].RFgain,
                ConfigData.rfGainAllBands);
  ...
  tft.fillRect(SMETER_X + 185, SMETER_Y, 80, tft.getFontHeight(),
               RA8875_BLACK); // The dB figure at end of S
  // DB2OO, 29-AUG-23: consider no decimals in the S-meter dBm value as it is
  // very busy with decimals
...
void ShowTempAndLoad() {
  ...
  block_time =
      ...
      (double)SR[SampleRate]
          .rate; // one audio block is 128 samples and uses this in seconds
  block_time = block_time * N_BLOCKS;

  block_time *= 1000000.0; // now in µseconds
  processor_load =
      elapsed_micros_mean / block_time *
      100; // take audio processing time divide by block_time, convert to %
  ...
  tft.fillRect(TEMP_X_OFFSET, TEMP_Y_OFFSET, MAX_WATERFALL_WIDTH,
               tft.getFontHeight(), RA8875_BLACK); // Erase current data
  ...
  MyDrawFloat(CPU_temperature, 1, TEMP_X_OFFSET + tft.getFontWidth() * 3,
              TEMP_Y_OFFSET, buff);
...
void MyDrawFloat(float val, int decimals, int x, int y, char *buff) {
  dtostrf(val, FLOAT_PRECISION, decimals,
          buff); // Use 8 as that is the max prevision on a float
...
FLASHMEM void UpdateInfoWindow() {
  tft.fillRect(INFORMATION_WINDOW_X - 8, INFORMATION_WINDOW_Y, 250, 170,
               RA8875_BLACK); // Clear fields
  tft.drawRect(BAND_INDICATOR_X - 10, BAND_INDICATOR_Y - 2, 260, 200,
               RA8875_LIGHT_GREY); // Redraw Info Window
}
...
void UpdateAudioField() {
  ...
  //  tft.setTextColor(RA8875_GREEN);
  //  tft.fillRect(BAND_INDICATOR_X + 90, BAND_INDICATOR_Y, tft.getFontWidth() *
  //  3 + 2, tft.getFontHeight(), RA8875_BLACK); tft.setCursor(FIELD_OFFSET_X,
  //  BAND_INDICATOR_Y); tft.print(ConfigData.audioVolume);
}
...
void UpdateVolumeField() {
  ...
  tft.print("Vol:");
  tft.setTextColor(RA8875_GREEN);
  tft.fillRect(BAND_INDICATOR_X + 80, BAND_INDICATOR_Y,
               tft.getFontWidth() * 3 + 2, tft.getFontHeight(), RA8875_BLACK);
...
void UpdateAGCField() {
  tft.setFontScale((enum RA8875tsize)1);
  tft.fillRect(AGC_X_OFFSET - 10, AGC_Y_OFFSET, tft.getFontWidth() * 7 + 2,
               tft.getFontHeight(), RA8875_BLACK);
...
/*****
  Purpose: Updates the AGC on the display.  Long option added. G0ORX September
6, 2023
...
void UpdateAGCField() {
  tft.setFontScale((enum RA8875tsize)1);
  tft.fillRect(AGC_X_OFFSET - 10, AGC_Y_OFFSET, tft.getFontWidth() * 7,
tft.getFontHeight(), RA8875_BLACK); tft.setCursor(BAND_INDICATOR_X + 133,
BAND_INDICATOR_Y); switch (ConfigData.AGCMode) {  // The option for AGC case 0:
// Off
                                 //    tft.setCursor(BAND_INDICATOR_X + 140,
BAND_INDICATOR_Y); tft.setTextColor(DARKGREY); tft.print("AGC OFF");
      //      tft.setFontScale((enum RA8875tsize)0);
...
void DisplayIncrementField() {
  ...
  tft.fillRect(INCREMENT_X + 67, INCREMENT_Y, tft.getFontWidth() * 4,
               tft.getFontHeight(), RA8875_BLACK);
  tft.setCursor(FIELD_OFFSET_X - 23, INCREMENT_Y - 1);
  tft.setTextColor(RA8875_GREEN);
  tft.print(ConfigData.fineTuneStep);

  tft.fillRect(INCREMENT_X + 188, INCREMENT_Y, tft.getFontWidth() * 7 + 2,
               tft.getFontHeight(), RA8875_BLACK);
...
void UpdateNotchField() {
  ...
  tft.fillRect(NOTCH_X + 60, NOTCH_Y - 3, 150, tft.getFontHeight() + 5,
               RA8875_BLACK);
...
void UpdateCompressionField() // JJP 8/26/2023
{
  ...
  if (ConfigData.compressorFlag == true) { // JJP 8/26/2023
    tft.print("On  ");
    tft.print(ConfigData.micCompRatio,
              0); // This is the compression ratio of the Open Audio compressor.
                  // Greg KF5N July 22, 2024
  } else {
    tft.setCursor(FIELD_OFFSET_X, COMPRESSION_Y - 5);
    tft.print("Off");
  }
...
FLASHMEM void UpdateAudioGraphics() {
  ...
  tft.print("Decoder:");
  tft.setTextColor(RA8875_GREEN);
  //  tft.fillRect(DECODER_X + 90, DECODER_Y, tft.getFontWidth() * 20,
  //  tft.getFontHeight() + 2, RA8875_BLACK);
  tft.setCursor(FIELD_OFFSET_X, DECODER_Y - 5);
  tft.fillRect(FIELD_OFFSET_X, DECODER_Y - 5, 140, 17, RA8875_BLACK); // Erase

  // Update text and graphics.  Use a method of clearing everything, and then
  // add only what is needed.
  tft.writeTo(L2); // This has to be written to L2 or it will be erased by the
                   // audio spectrum eraser.
  tft.clearMemory();
  ...
  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER or
    ...
    filterLoPositionMarker =
        map(bands.bands[ConfigData.currentBand].FLoCut, 0, 6000, 0, 256);
    filterHiPositionMarker =
        map(bands.bands[ConfigData.currentBand].FHiCut, 0, 6000, 0, 256);
  } else if (bands.bands[ConfigData.currentBand].sideband ==
                 Sideband::BOTH_AM or
             bands.bands[ConfigData.currentBand].sideband ==
                 Sideband::BOTH_SAM) {
    filterHiPositionMarker =
        map(bands.bands[ConfigData.currentBand].FAMCut, 0, 6000, 0, 256);
  }

  // Draw Filter indicator lines on audio plot to Layer 2.
  tft.writeTo(L2);
  // The encoder should adjust the low side of the filter if the CW filter is
  // on.  This creates a tunable bandpass.  Sort of.
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and
      ConfigData.CWFilterIndex != 5)
    switchFilterSideband = true;

  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER or
      bands.bands[ConfigData.currentBand].sideband == Sideband::UPPER) {
    if (not switchFilterSideband) {
      tft.drawLine(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker),
                   SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker),
                   SPECTRUM_BOTTOM - 112, RA8875_LIGHT_GREY);
      tft.drawLine(BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker),
                   SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 7 + abs(filterHiPositionMarker),
                   SPECTRUM_BOTTOM - 112, RA8875_RED);
    } else {
      tft.drawLine(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker),
                   SPECTRUM_BOTTOM - 3,
                   BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker),
  ...
  // Select the x-axis position for the CW filter box and delimiter line (on the
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and
    ...
    tft.writeTo(L2); // This has to be written to L2 or it will be erased by the
                     // audio spectrum eraser.
    tft.fillRect(BAND_INDICATOR_X - 6 + abs(filterLoPositionMarker),
                 AUDIO_SPECTRUM_TOP,
                 CWFilterPosition - abs(filterLoPositionMarker), 120, MAROON);
    tft.drawFastVLine(BAND_INDICATOR_X - 8 + CWFilterPosition,
                      AUDIO_SPECTRUM_BOTTOM - 118, 118, RA8875_LIGHT_GREY);
  }

  // Draw decoder delimiters if the decoder is on.
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::CW_MODE and
      ConfigData.decoderFlag) {
    // Draw delimiter bars for CW offset frequency.  This depends on the user
    // selected offset.
    if (ConfigData.CWOffset == 0) {
      tft.drawFastVLine(BAND_INDICATOR_X + 15, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 21, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW upper freq indicator
    }
    if (ConfigData.CWOffset == 1) {
      tft.drawFastVLine(BAND_INDICATOR_X + 18, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 24, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW upper freq indicator
    }
    if (ConfigData.CWOffset == 2) {
      tft.drawFastVLine(BAND_INDICATOR_X + 23, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 29, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW upper freq indicator
    }
    if (ConfigData.CWOffset == 3) {
      tft.drawFastVLine(BAND_INDICATOR_X + 26, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW lower freq indicator
      tft.drawFastVLine(BAND_INDICATOR_X + 32, AUDIO_SPECTRUM_BOTTOM - 118, 118,
                        RA8875_GREEN); // CW upper freq indicator
    }
  }
...
FLASHMEM void UpdateEqualizerField(bool rxEqState, bool txEqState) {
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(FIELD_OFFSET_X, DECODER_Y + 15, tft.getFontWidth() * 15,
               tft.getFontHeight() + 2, RA8875_BLACK);
  tft.setTextColor(RA8875_WHITE); // Display zoom factor
...
void UpdateWPMField() {
  ...
  tft.print("Keyer:");
  tft.setTextColor(RA8875_GREEN);
  tft.fillRect(WPM_X + 59, WPM_Y - 4, tft.getFontWidth() * 15,
               tft.getFontHeight(), RA8875_BLACK);
...
void UpdateNoiseField() {
  ...
  tft.setFontScale((enum RA8875tsize)0);
  tft.fillRect(FIELD_OFFSET_X, NOISE_REDUCE_Y, 35, tft.getFontHeight(),
               RA8875_BLACK);
...
void DrawBandWidthIndicatorBar() // AFP 10-30-22
{
  ...
  case 0: // 1X
    hz_per_pixel = 375.0;
    Zoom1Offset = -128; // The "center" is halfway to the left of the spectrum
                        // window center.  // KF5N April 23, 2024
    break;
  ...
  newCursorPosition =
      static_cast<int>(NCOFreq / hz_per_pixel) +
      Zoom1Offset; // More accurate tuning bar position.  KF5N May 17, 2024
  tft.writeTo(L2); // Write graphics to Layer 2.
  //  tft.clearMemory();              // This destroys the CW filter graphics,
  //  removed.  KF5N July 30, 2023 tft.clearScreen(RA8875_BLACK);  // This
  //  causes an audio hole in fine tuning.  KF5N 7-16-23

  pixel_per_khz = ((1 << ConfigData.spectrum_zoom) * SPECTRUM_RES * 1000.0 /
                   SR[SampleRate].rate);
  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER or
      bands.bands[ConfigData.currentBand].sideband == Sideband::UPPER)
    filterWidth =
        static_cast<int>(((bands.bands[ConfigData.currentBand].FHiCut -
                           bands.bands[ConfigData.currentBand].FLoCut) /
                          1000.0) *
                         pixel_per_khz * 1.06); // AFP 10-30-22
  else if (bands.bands[ConfigData.currentBand].sideband == Sideband::BOTH_AM or
           bands.bands[ConfigData.currentBand].sideband == Sideband::BOTH_SAM)
    filterWidth = static_cast<int>(
        ((bands.bands[ConfigData.currentBand].FAMCut * 2.0) / 1000.0) *
        pixel_per_khz * 1.06); // AFP 10-30-22

  switch (bands.bands[ConfigData.currentBand].sideband) {
  case Sideband::LOWER:
    tft.fillRect(centerLine - filterWidth + oldCursorPosition,
                 SPECTRUM_TOP_Y + 20, filterWidth * 1.0, SPECTRUM_HEIGHT - 20,
                 RA8875_BLACK); // Was 0.96.  KF5N July 31, 2023
    tft.fillRect(centerLine - filterWidth + newCursorPosition,
                 SPECTRUM_TOP_Y + 20, filterWidth, SPECTRUM_HEIGHT - 20,
                 FILTER_WIN);
    break;

  case Sideband::UPPER:
    tft.fillRect(centerLine + oldCursorPosition, SPECTRUM_TOP_Y + 20,
                 filterWidth, SPECTRUM_HEIGHT - 20,
                 RA8875_BLACK); // AFP 03-27-22 Layers
    tft.fillRect(
        centerLine + newCursorPosition, SPECTRUM_TOP_Y + 20, filterWidth,
        SPECTRUM_HEIGHT - 20,
        FILTER_WIN); // AFP 03-27-22 Layers //      tft.drawFastVLine(centerLine
                     // + oldCursorPosition, SPECTRUM_TOP_Y + 20, h - 10,
                     // RA8875_BLACK); // Yep. Erase old, draw new...//AFP
                     // 03-27-22 Layers //      tft.drawFastVLine(centerLine +
                     // newCursorPosition , SPECTRUM_TOP_Y + 20, h - 10,
                     // RA8875_CYAN); //AFP 03-27-22 Layers
    break;

  case Sideband::BOTH_AM:
  case Sideband::BOTH_SAM:
    tft.fillRect(centerLine - filterWidth / 2 + oldCursorPosition,
                 SPECTRUM_TOP_Y + 20, filterWidth, SPECTRUM_HEIGHT - 20,
                 RA8875_BLACK); // AFP 10-30-22
    tft.fillRect(
        centerLine - filterWidth / 2 * 0.93 + newCursorPosition,
        SPECTRUM_TOP_Y + 20, filterWidth * 0.95, SPECTRUM_HEIGHT - 20,
        FILTER_WIN); // AFP 10-30-22
                     //       tft.drawFastVLine(centerLine + newCursorPosition,
                     //       SPECTRUM_TOP_Y + 20, h - 10, RA8875_CYAN); //AFP
                     //       10-30-22*/
    break;
  ...
  tft.drawFastVLine(centerLine + oldCursorPosition, SPECTRUM_TOP_Y + 20, h - 10,
                    RA8875_BLACK); // refactored from above JJP 7/12/23
  tft.drawFastVLine(centerLine + newCursorPosition, SPECTRUM_TOP_Y + 20, h - 10,
                    RA8875_CYAN);
...
void EraseSpectrumDisplayContainer() {
  tft.fillRect(SPECTRUM_LEFT_X - 2, SPECTRUM_TOP_Y - 1, MAX_WATERFALL_WIDTH + 6,
               SPECTRUM_HEIGHT + 8, RA8875_BLACK); // Spectrum box
}
...
void EraseSpectrumWindow() {
  tft.fillRect(SPECTRUM_LEFT_X, SPECTRUM_TOP_Y, MAX_WATERFALL_WIDTH,
               SPECTRUM_HEIGHT, RA8875_BLACK); // Spectrum box
}
...
*****/
void EraseMenus() {
  tft.fillRect(PRIMARY_MENU_X, MENUS_Y, BOTH_MENU_WIDTHS, CHAR_HEIGHT + 1,
               RA8875_BLACK); // Erase menu choices
}
...
void ErasePrimaryMenu() {
  tft.fillRect(PRIMARY_MENU_X, MENUS_Y, EACH_MENU_WIDTH, CHAR_HEIGHT + 1,
               RA8875_BLACK); // Erase menu choices
...
void EraseSecondaryMenu() {
  tft.fillRect(SECONDARY_MENU_X, MENUS_Y, EACH_MENU_WIDTH, CHAR_HEIGHT + 1,
               RA8875_BLACK); // Erase menu choices
  //  menuStatus = NO_MENUS_ACTIVE; // Change menu state
}
...
void ShowTransmitReceiveStatus() {
  ...
  if (radioState == RadioState::SSB_TRANSMIT_STATE or
      radioState == RadioState::FT8_TRANSMIT_STATE or
      radioState == RadioState::CW_TRANSMIT_STRAIGHT_STATE or
      radioState == RadioState::CW_TRANSMIT_KEYER_STATE or
      radioState == RadioState::CW_CALIBRATE_STATE or
