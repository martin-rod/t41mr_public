...
constexpr int RAY_LENGTH = 190;

// Set for call prefixes. May be increased later
constexpr size_t keyboardBufferSize = 11;
char keyboardBuffer[keyboardBufferSize] = {0};
...
PROGMEM const int spacing[] = {0, 70, 140, 210, 280, 350, 420, 490, 560, 630};

// Map name and coordinates
constexpr size_t mapsSize = 10;
size_t mapsCount = 0;
size_t mapsOnCardCount = 0;
...
maps_t mapFiles[mapsSize] = {
    {.fileName = "", .latitude = 0.0, .longitude = 0.0, .isOnCard = false},
    {.fileName = "", .latitude = 0.0, .longitude = 0.0, .isOnCard = false},
    {.fileName = "", .latitude = 0.0, .longitude = 0.0, .isOnCard = false},
    {.fileName = "", .latitude = 0.0, .longitude = 0.0, .isOnCard = false},
    {.fileName = "", .latitude = 0.0, .longitude = 0.0, .isOnCard = false},
...
/*****
  Purpose: The next two functions read 16- and 32-bit image data from the SD
card file. BMP data is stored little-endian, Arduino is little-endian too.
...
FLASHMEM void DrawKeyboard() {
  ...
  tft.setCursor(10, 0);
  tft.printf("[Menu Down] Enter highlighted char");
  tft.setCursor(10, 30);
  tft.printf("[Select, Bearing] Done");
  tft.setCursor(10, 60);
  tft.printf("[Band UP] Erase last char");
...
FLASHMEM void CaptureKeystrokes() {
  ...
  while (true) {
    ...
    case MenuSelect::MAIN_MENU_DN:
      ...
      if (row < 240) {
        keyboardBuffer[keyboardBufferIndex] = whichLetterIndex;
      } else {
        keyboardBuffer[keyboardBufferIndex] = letters[whichLetterIndex];
      }

      if (keyboardBufferIndex < keyboardBufferSize - 1) {
        keyboardBufferIndex++;
      }
    ...
    case MenuSelect::BAND_UP:
      ...
      keyboardBuffer[keyboardBufferIndex] = '\0'; // Make it a string
...
/*****
  ...
  Parameter list:
    ...
    int whichLetterIndex      which letter is being drawn from the letters[]
array int keyWidth,             the width of the drawn key int keyHeight height
...
/*****
  Purpose: To draw the letters for the onscreen keyboard, but highlight it when
drawn
 ...
 *****/
FLASHMEM bool BearingHeading(char *dxCallPrefix, float &heading, float &distance, float &dxLat, float &dxLon) {
  float deltaLong; // For radians conversion
  float x, y;      // Temporary variables

  countryIndex = FindCountry(dxCallPrefix);

  tft.clearScreen(RA8875_BLACK);

  if (countryIndex != -1) {
    dxLat = dxCities[countryIndex].lat;
  ...
  tft.setFontScale((enum RA8875tsize)1);
  deltaLong = (ConfigData.myLong - dxLon);

  x = cos(dxLat * DEGREES2RADIANS) * sin(deltaLong * DEGREES2RADIANS);
  y = cos(ConfigData.myLat * DEGREES2RADIANS) * sin(dxLat * DEGREES2RADIANS) -
      sin(ConfigData.myLat * DEGREES2RADIANS) * cos(dxLat * DEGREES2RADIANS) * cos((deltaLong)*DEGREES2RADIANS);
  distance = HaversineDistance(dxLat, dxLon);

  float bearingDegrees = atan2(x, y) * RADIANS2DEGREES;
  bearingDegrees = fmod(bearingDegrees, 360.0);

  if (bearingDegrees > 0) {
    heading = 360.0 - bearingDegrees;
  } else {
    heading = bearingDegrees * -1.0;
  }
  return true;
}
/*****
...
// calculates the distance the distance in km. Multiply by 0.6213712 for miles
...
FLASHMEM float HaversineDistance(float lat2, float lon2) {
  ...
  lat1 = ConfigData.myLat;
  lon1 = ConfigData.myLong;

  const float a = 0.5 - cos((lat2 - lat1) * p) / 2 + cos(lat1 * p) * cos(lat2 * p) * (1 - cos((lon2 - lon1) * p)) / 2;

  return 2.0 * r * asin(sqrt(a));
}

FLASHMEM bool DrawBmpCenter(const char *filename, std::string &errorMsg) {
  TRACE_LEVEL(TR_L_TRACE);

  File bmpFile;

  uint16_t bmpSignature;
  uint16_t bmpPlanes;
  uint16_t bmpDepth;       // Bit depth (currently must be 24)
  uint32_t bmpImageoffset; // Start of image data in file
  uint32_t bmpCompresion;
  ...
  TRACE_T41(TR_L_INFO, "displayWidth:%d displayHeight:%d", displayWidth, displayHeight);
  ...
  // Parse BMP header
  bmpSignature = read16(bmpFile);
  if (bmpSignature != 0x4D42) {
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "bad bmpSignature:0x%04X", bmpSignature);
    errorMsg = buf;
    goto CloseAndError;
  }
  ...
  TRACE_T41(TR_L_INFO, "bmpWidth:%d bmpHeight:%d", bmpWidth, bmpHeight);

  bmpPlanes = read16(bmpFile);
  if (bmpPlanes != 1) {
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "bad bmpPlanes:%u", bmpPlanes);
    errorMsg = buf;
    goto CloseAndError;
  }
  bmpDepth = read16(bmpFile); // bits per pixel

  // 24=3x8 without alpha
  if (bmpDepth != 24) {
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "bad bmpDepth:%u", bmpDepth);
    errorMsg = buf;
    goto CloseAndError;
  }

  bmpCompresion = read32(bmpFile);
  if (bmpCompresion != 0) {
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "bad bmpCompresion:%ld", bmpCompresion);
    errorMsg = buf;
    goto CloseAndError;
  }
  ...
  if (bmpHeight > displayWidth || bmpHeight > displayHeight) {
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "x or y out of range x:%d y:%d", x, y);
    errorMsg = buf;
    goto Error;
  }
...
CloseAndError:
  bmpFile.close();
Error:
  TRACE_T41(TR_L_ERROR, "%s", errorMsg.c_str());
  return false;
}

FLASHMEM bool DrawBmp(const char *filename, int x, int y, int &bmpWidth, int &bmpHeight, std::string &errorMsg) {
  TRACE_LEVEL(TR_L_TRACE);
...
FLASHMEM void DrawEmptyMap(const char *filename) {
  TRACE_LEVEL(TR_L_TRACE);
  std::string errorMsg;

  bool bmpOk = DrawBmpCenter(filename, errorMsg);
  if (not bmpOk) {
    tft.setCursor(100, 300);
    tft.printf(errorMsg.c_str());
    TRACE_T41(TR_L_ERROR, "DrawBmp() filename:%s", filename);
    delay(2000L);
    return;
  }
}

FLASHMEM void DrawMap(const char *filename, float bearingHeading, float distance, float dxLat, float dxLon) {
  TRACE_LEVEL(TR_L_TRACE);
  std::string errorMsg;
  ...
  ConfigData.myLat = mapFiles[selectedMapIndex].latitude;
  ConfigData.myLong = mapFiles[selectedMapIndex].longitude;

  float homeLatRadians;
  float dxLatRadians;
  float deltaLon = (dxLon - ConfigData.myLong);
  float deltaLonRadians;

  //======================================
  homeLatRadians = ConfigData.myLat * DEGREES2RADIANS;
  ...
  float bearingRadians = atan2(yR, xR);
  float bearingDegrees = bearingRadians * RADIANS2DEGREES;

  TRACE_T41(TR_L_TRACE, "bearingDegrees:%f", bearingDegrees);

  uint16_t displayWidth = tft.width();
  uint16_t displayHeight = tft.height();

  uint16_t centerX = displayWidth / 2 - 1;
  uint16_t centerY = displayHeight / 2 - 1;

  float displayBearing = bearingHeading;

  rayStart = displayBearing - 8.0;
  rayEnd = displayBearing + 8.0;

  // Check for end-point mapping issues
  if (displayBearing > 16 && displayBearing < 345) {
    rayStart = -8;
    rayEnd = 9;
  } else {
    if (displayBearing < 9) {
  ...
  for (int i = rayStart; i < rayEnd; i++) {
    tft.drawLineAngle(centerX, centerY, displayBearing + i, RAY_LENGTH, RA8875_RED, -90);
  }
  ...
  tft.printf("%.0f\xB0", displayBearing);
  tft.setCursor(20, 400);
  tft.printf("%.0f km", distance);
  tft.setCursor(20, 440);
  tft.printf("%.0f miles", distance * 0.6213712);
}

/*****
  Purpose: Function takes the RBG color values and converts them for their BCD
format to the equivalent tft color value.
...
FLASHMEM void LoadMapFiles(const char *filename) {
  TRACE_LEVEL(TR_L_TRACE);
  ...
  if (error) {
    ...
    TRACE_T41(TR_L_ERROR, "error:%s", error.c_str());
  ...
  for (JsonPair kv : doc.as<JsonObject>()) {
    TRACE_T41(TR_L_INFO, "key:%s", kv.key().c_str());
  }

  TRACE_T41(TR_L_INFO, "read filename:%s", filename);

  if (not doc.is<JsonArray>()) {
    TRACE_T41(TR_L_ERROR, "Json doc is not array");
    return;
  }

  size_t docSize = doc.size();
  TRACE_T41(TR_L_INFO, "doc.size:%u", docSize);

  size_t mapsInd = 0;
  for (size_t i = 0; i < mapsSize; i++) {
    if (i >= docSize) {
      TRACE_T41(TR_L_TRACE, "break docSize :%u", docSize);
      break;
    }

    if (doc[i]["fileName"].is<const char *>()) {
      strlcpy(mapFiles[mapsInd].fileName, doc[i]["fileName"], filenameSize);
      TRACE_T41(TR_L_TRACE, "fileName:%s", mapFiles[i].fileName);
    } else {
      TRACE_T41(TR_L_ERROR, "mapFiles[%u].fileName not found", i);
      break;
    }

    if (doc[i]["qthLatitude"].is<float>()) {
      mapFiles[mapsInd].latitude = doc[i]["qthLatitude"];
      TRACE_T41(TR_L_TRACE, "qthLatitude:%f", mapFiles[i].latitude);
    } else {
      TRACE_T41(TR_L_ERROR, "mapFiles[%u].qthLatitude not found", i);
      break;
    }

    if (doc[i]["qthLongitude"].is<float>()) {
      mapFiles[mapsInd].longitude = doc[i]["qthLongitude"];
      TRACE_T41(TR_L_TRACE, "qthLongitude:%f", mapFiles[i].longitude);
    } else {
      TRACE_T41(TR_L_ERROR, "mapFiles[%u].qthLongitude not found", i);
      break;
    }
...
FLASHMEM int CheckMapList(maps_t mapFiles[], size_t mapCount) {
  ...
  for (size_t i = 0; i < mapCount; i++) {
    if (std::find(fileList.begin(), fileList.end(), mapFiles[i].fileName) != fileList.end()) {
      mapFiles[i].isOnCard = true;
      foundCnt++;
    }
  }
  return foundCnt;
...
int WhichOneToUse(maps_t mapFiles[], size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (mapFiles[i].isOnCard) {
      tft.printf("%02d %20s", i, mapFiles[i].fileName);
    } else {
      tft.printf("%02d %20s", i, "---");
    }
  }

  tft.setCursor(50, 55);
  tft.setTextColor(RA8875_BLACK, RA8875_GREEN);
  if (mapFiles[0].isOnCard) {
    tft.printf("%02d %20s", 0, mapFiles[0].fileName);
  } else {
    tft.printf("%02d %20s", 0, "---");
  }

  while (true) {
    delay(100L);
    if (filterEncoderMove != 0) {
      // Restore old highlighted name
      tft.setCursor(50, 55 + selected * 30);
      tft.setTextColor(RA8875_WHITE, RA8875_BLACK);

      if (mapFiles[selected].isOnCard) {
        tft.printf("%02d %20s", selected, mapFiles[selected].fileName);
      } else {
        tft.printf("%02d %20s", selected, "---");
      }

      selected += filterEncoderMove;
      ...
      TRACE_T41(TR_L_TRACE, "selected:%d", selected);
    ...
    if (menu == MenuSelect::MENU_OPTION_SELECT) {
      if (not mapFiles[selected].isOnCard) {
        selected = -1;
      }
      break;
    }
  ...
  tft.setTextColor(RA8875_WHITE, RA8875_BLACK);
  TRACE_T41(TR_L_DEBUG, "selected:%d", selected);
  return selected;
}
...
FLASHMEM void CheckMapList() {
  ...
  mapsOnCardCount = CheckMapList(mapFiles, mapsCount);

  selectedMapIndex = -1;
  if (mapsOnCardCount == 0) {
    tft.setCursor(300, 300);
    tft.print("No Maps found");
    TRACE_T41(TR_L_ERROR, "No Maps found");
    return;
  }

  if (strlen(ConfigData.mapFileName) > 0) {
    for (size_t i = 0; i < mapsCount; i++) {
      if (mapFiles[i].isOnCard == true) {
        if (strncmp(mapFiles[i].fileName, ConfigData.mapFileName, filenameSize) == 0) {
          selectedMapIndex = i;
          ConfigData.myLat = mapFiles[selectedMapIndex].latitude;
          ConfigData.myLong = mapFiles[selectedMapIndex].longitude;
          TRACE_T41(TR_L_INFO, "selectedMapIndex:%d", selectedMapIndex);
          TRACE_T41(TR_L_INFO, "Selected fileName:%s", ConfigData.mapFileName);
          TRACE_T41(TR_L_INFO, "Selected myLat:%f myLong:%f", ConfigData.myLat, ConfigData.myLong);
          break;
        }
      }
    }
...
FLASHMEM void SelectBearingMap() {
  ...
  TRACE_T41(TR_L_TRACE, "mapsCount:%u mapsOnCardCount:%u", mapsCount, mapsOnCardCount);

  if (mapsOnCardCount == 0) {
    TRACE_T41(TR_L_ERROR, "No valid maps faile on card");
    tft.setCursor(200, 300);
    tft.setTextColor(RA8875_RED, RA8875_BLACK);
    tft.println("No SD card.");
    delay(3000);
    ...
    selectedMapIndex = WhichOneToUse(mapFiles, mapsCount);

    if (selectedMapIndex == -1) {
      tft.setCursor(300, 300);
      tft.print("No Maps selected");
      goto restoreScreen;
    }
  }
  TRACE_T41(TR_L_INFO, "selectedMapIndex:%d", selectedMapIndex);
  ...
  ConfigData.myLat = mapFiles[selectedMapIndex].latitude;
  ConfigData.myLong = mapFiles[selectedMapIndex].longitude;
  strlcpy(ConfigData.mapFileName, (const char *)mapFiles[selectedMapIndex].fileName, ConfigDataMapFileNameSize);
...
void DrawBearingMap(void) {
  ...
  if (selectedMapIndex < 0 || selectedMapIndex >= static_cast<int>(mapsSize)) {
    TRACE_T41(TR_L_ERROR, "selectedMapIndex:%d", selectedMapIndex);
    tft.setCursor(300, 300);
    tft.print("No selected map");
    delay(3000);
    goto restoreScreen;
  }

  TRACE_T41(TR_L_TRACE, "selectedMapIndex:%d", selectedMapIndex);
  DrawKeyboard();
  CaptureKeystrokes();
  TRACE_T41(TR_L_TRACE, "keyboardBuffer:%s", keyboardBuffer);
  ...
  if (keyboardBuffer[0] == 0) {
    ...
    DrawEmptyMap((char *)mapFiles[selectedMapIndex].fileName);
  } else {
    float heeading = 0.0;
    float distance = 0.0;
    float dxLat = 0.0;
    float dxLot = 0.0;
    bool headingFound = BearingHeading(keyboardBuffer, heeading, distance, dxLat, dxLot);

    if (not headingFound) {
      int16_t x = 380 - (17 * tft.getFontWidth(0)) / 2;
      tft.setCursor(x, 240);
      tft.print("Country not found");
      tft.setCursor(x, 240 + 100);
      tft.printf("country:%s", keyboardBuffer);
      delay(3000);
      goto restoreScreen;
    }
    TRACE_T41(TR_L_TRACE, "heeading:%f, distance:%f, dxLat:%f, dxLot:%f", heeading, distance, dxLat, dxLot);
    ...
    DrawMap((char *)mapFiles[selectedMapIndex].fileName, heeading, distance, dxLat, dxLot);
  }
  ...
  while (not doneViewing) {
    ...
    case MenuSelect::BEARING:
      doneViewing = true;
      break;
    case MenuSelect::BOGUS_PIN_READ:
    default:
      break;
    }
  }
