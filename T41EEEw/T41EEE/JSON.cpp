...
// JSON format used to save and read from SD card.  This was derived from a JSON
// example from the ArduinoJSON library.
...
FLASHMEM void JSON::loadConfiguration(const char *filename,
                                      config_t &ConfigData) {
  ...
  //  constant.  This does not need to be included in user data.
...
// Saves the configuration ConfigData to a file or writes to serial.  toFile ==
// true for file, false for serial.
FLASHMEM void JSON::saveConfiguration(const char *filename,
                                      const config_t &ConfigData, bool toFile) {
  ...
  doc["versionSettings"] =
      ConfigData.versionSettings; // Fix for version not updating in JSON file.
                                  // KF5N March 18, 2024.
  ...
  //  for (int i = 0; i < 18; i++) doc["switchValues"][i] =
  //  ConfigData.switchValues[i];
  ...
  doc["SSBPowerCalibrationFactor"][i] = ConfigData.SSBPowerCalibrationFactor[i];
  for (int i = 0; i < 7; i++) doc["IQCWRXAmpCorrectionFactor"][i] =
  ConfigData.IQCWRXAmpCorrectionFactor[i]; for (int i = 0; i < 7; i++)
  doc["IQCWRXPhaseCorrectionFactor"][i] =
  ConfigData.IQCWRXPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
  doc["IQCWAmpCorrectionFactor"][i] =   ConfigData.IQCWAmpCorrectionFactor[i];
  for (int i = 0; i < 7; i++) doc["IQCWPhaseCorrectionFactor"][i] =
  ConfigData.IQCWPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
  doc["IQSSBRXAmpCorrectionFactor"][i] =
  ConfigData.IQSSBRXAmpCorrectionFactor[i]; for (int i = 0; i < 7; i++)
  doc["IQSSBRXPhaseCorrectionFactor"][i] =
  ConfigData.IQSSBRXPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
  doc["IQSSBAmpCorrectionFactor"][i] =   ConfigData.IQSSBAmpCorrectionFactor[i];
  ...
  */
  ...
  for (int i = 0; i < 7; i++) doc["iDCoffsetSSB"][i] =
  ConfigData.iDCoffsetSSB[i]; for (int i = 0; i < 7; i++) doc["qDCoffsetSSB"][i]
  = ConfigData.qDCoffsetSSB[i]; doc["dacOffsetCW"] = ConfigData.dacOffsetCW;
...
// Loads the ConfigData configuration from a file
FLASHMEM void JSON::loadCalibration(const char *filename,
  ...
  /*
    ...
    CalData.IQCWRXPhaseCorrectionFactor[i] =
    doc["IQCWRXPhaseCorrectionFactor"][i]; for (int i = 0; i < 7; i++)
    CalData.IQCWAmpCorrectionFactor[i] = doc["IQCWAmpCorrectionFactor"][i]; for
    (int i = 0; i < 7; i++) CalData.IQCWPhaseCorrectionFactor[i] =
    doc["IQCWPhaseCorrectionFactor"][i]; for (int i = 0; i < 7; i++)
    CalData.IQSSBRXAmpCorrectionFactor[i] =
    doc["IQSSBRXAmpCorrectionFactor"][i]; for (int i = 0; i < 7; i++)
    CalData.IQSSBRXPhaseCorrectionFactor[i] =
    doc["IQSSBRXPhaseCorrectionFactor"][i]; for (int i = 0; i < 7; i++)
    CalData.IQSSBAmpCorrectionFactor[i] = doc["IQSSBAmpCorrectionFactor"][i];
...
// Saves the configuration CalData to a file or writes to serial.  toFile ==
// true for file, false for serial.
FLASHMEM void JSON::saveCalibration(const char *filename,
                                    const calibration_t &CalData, bool toFile) {
  ...
  /*
    ...
    doc["IQCWRXPhaseCorrectionFactor"][i] =
    CalData.IQCWRXPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
    doc["IQCWAmpCorrectionFactor"][i] =   CalData.IQCWAmpCorrectionFactor[i];
    for (int i = 0; i < 7; i++) doc["IQCWPhaseCorrectionFactor"][i] =
    CalData.IQCWPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
    doc["IQSSBRXAmpCorrectionFactor"][i] =
    CalData.IQSSBRXAmpCorrectionFactor[i]; for (int i = 0; i < 7; i++)
    doc["IQSSBRXPhaseCorrectionFactor"][i] =
    CalData.IQSSBRXPhaseCorrectionFactor[i]; for (int i = 0; i < 7; i++)
    doc["IQSSBAmpCorrectionFactor"][i] =   CalData.IQSSBAmpCorrectionFactor[i];
    for (int i = 0; i < 7; i++) doc["IQSSBPhaseCorrectionFactor"][i] =
