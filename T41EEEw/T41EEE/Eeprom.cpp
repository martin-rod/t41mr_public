...
/*****
  ...
  CAUTION: This code assumes you have set the curently active VFO frequency to
the new frequency you wish to save. You them use the menu encoder to scroll
through the current list of stored frequencies. Stop on the one that you wish to
...
void Eeprom::SetFavoriteFrequency() {
  ...
  while (true) {
    ...
    //    switches val = ProcessButtonPress(val);
    menu = readButton();
    delay(150L);
...
/*****
  Purpose: Used to fetch a favortie frequency as stored in EEPROM. It then
copies that frequency to the currently active VFO
...
FLASHMEM void Eeprom::GetFavoriteFrequency() {
  ...
  while (true) {
    ...
    } else if (ConfigData.centerFreq >= bands.bands[BAND_15M].fBandLow &&
               ConfigData.centerFreq <= bands.bands[BAND_15M].fBandHigh) {
      currentBand2 = BAND_15M;
    } else if (ConfigData.centerFreq >= bands.bands[BAND_12M].fBandLow &&
               ConfigData.centerFreq <= bands.bands[BAND_12M].fBandHigh) {
      currentBand2 = BAND_12M;
    } else if (ConfigData.centerFreq >= bands.bands[BAND_10M].fBandLow &&
               ConfigData.centerFreq <= bands.bands[BAND_10M].fBandHigh) {
...
bool Eeprom::EEPROMStartup() {
  ...
  // configured by the user, and are not to be lost. However, if the ConfigData
  // or the CalData struct changes, it is necessary to overwrite the EEPROM with
  // the new struct. This decision is made by using a simple size comparison.
  // This is not fool-proof, but it will probably work most of the time.  The
  // users should be instructed to always save the EEPROM (ConfigData and
  // CalData) to SD for later recovery of their calibration and configuration
