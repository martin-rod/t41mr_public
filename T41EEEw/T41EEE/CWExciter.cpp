...
/*****
  Purpose: CW Key interrupt  - to send a Morse code dit
...
void CW_ExciterIQData(int shaping) // AFP 08-20-22
{
  ...
  if (bands.bands[ConfigData.currentBand].sideband == Sideband::LOWER) {
    // Adjust level of L buffer KF5N flipped sign, original was +
    arm_scale_f32(float_buffer_L_EX, -CalData.IQCWAmpCorrectionFactorLSB[ConfigData.currentBand], float_buffer_L_EX, 256);
    // Adjust phase
    IQPhaseCorrection(float_buffer_L_EX, float_buffer_R_EX, CalData.IQCWPhaseCorrectionFactorLSB[ConfigData.currentBand], 256);
  } else {
    if (bands.bands[ConfigData.currentBand].sideband == Sideband::UPPER) {
      // KF5N flipped sign, original was minus.
      arm_scale_f32(float_buffer_L_EX, +CalData.IQCWAmpCorrectionFactorUSB[ConfigData.currentBand], float_buffer_L_EX, 256);
      // Adjust phase
...
// This is the correct place in the data stream to inject the scaling for power.
  ...
  **********************************************************************************/
  ...
  // source, destination, number of samples
  arm_float_to_q15(float_buffer_Sidetone, q15_buffer_Sidetone, 2048);
...
// Inject the DC offset from carrier calibration.  There is an ARM function for this.
  ...
  // This mode is required when setting sidetone volume. The process will stall due to disconnection of I and Q patchcords.
  Q_out_L_Ex.setBehaviour(AudioPlayQueue::NON_STALLING);
  Q_out_R_Ex.setBehaviour(AudioPlayQueue::NON_STALLING);

  // Transmitter CW sidetone.  Connected to receiver audio path during transmit.
