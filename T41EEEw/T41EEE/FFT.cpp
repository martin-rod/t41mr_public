...
void ZoomFFTExe(uint32_t blockSize) {
  ...
  for (int i = 0; i < sample_no; i++) { // interleave real and imaginary input
                                        // values [real, imag, real, imag . . .]
  ...
  // so we calculate an FFT and display the spectrum every time this function is
  // called?
...
void CalcZoom1Magn() {
  if (updateDisplayFlag == 1) {
    ...
    // we do not need to calculate magnitudes with square roots, it would seem
    // to be sufficient to calculate mag = I*I + Q*Q, because we are doing a
    // log10-transformation later anyway and simultaneously put them into the
    // right order 38.50%, saves 0.05% of processor power and 1kbyte RAM ;-)
