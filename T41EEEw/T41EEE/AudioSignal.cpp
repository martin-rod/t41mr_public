...
AudioEffectGain_F32 compGainCompensate(audio_settings); // Microphone gain control.
...
// Converts Float to Int16.  See class in AudioStream_F32.h
...
AudioMixer4_F32 mixer2_tx;
AudioMixer4_F32 mixer3_tx;

// Tones for SSB calibration and IMD testing.
AudioSynthWaveformSine_F32 toneSSBCal1;
AudioSynthWaveformSine_F32 toneSSBCal2;

// AudioRecordQueue for input Microphone channel.
AudioRecordQueue Q_in_L_Ex;
// This 2nd channel is needed as we are bringing I and Q into the sketch instead of only microphone audio.
...
// Microphone audio channel.  Must use int2Float because Open Audio does not have quad input.
AudioConnection connect0(i2s_quadIn, 0, int2Float1_tx, 0);

// Used switches here because it appeared necessary to stop flow of data.
AudioConnection_F32 connect1(int2Float1_tx, 0, switch1_tx, 0);

// Tone used during SSB calibration.
...
AudioEffectGain_F32 speakerVolume;
AudioEffectGain_F32 speakerScale;
AudioEffectGain_F32 headphoneVolume;
AudioEffectGain_F32 headphoneScale;
AudioEffectGain_F32 compGain;
...
void SetAudioOperatingState(RadioState operatingState) {
  ...
  case RadioState::CW_TRANSMIT_KEYER_STATE:
    ...
    sgtl5000_1.volume(static_cast<float32_t>(ConfigData.sidetoneHeadphone) / 100.0); // This is for scaling the sidetone in the
    ...
    // Adjust sidetone volume. Headphone sidetone done in hardware.
