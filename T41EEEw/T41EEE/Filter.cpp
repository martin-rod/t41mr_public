...
void FilterBandwidth() {
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::SSB_MODE or
    ...
    case Sideband::LOWER:
      CalcCplxFIRCoeffs(FIR_Coef_I, FIR_Coef_Q, m_NumTaps, static_cast<float>(-bands.bands[ConfigData.currentBand].FHiCut),
                        static_cast<float>(-bands.bands[ConfigData.currentBand].FLoCut),
                        static_cast<float>(SR[static_cast<size_t>(SampleRate)].rate / DF));
      break;

    case Sideband::UPPER:
      CalcCplxFIRCoeffs(FIR_Coef_I, FIR_Coef_Q, m_NumTaps, static_cast<float>(bands.bands[ConfigData.currentBand].FLoCut),
  ...
  if (bands.bands[ConfigData.currentBand].mode == RadioMode::AM_MODE or
    ...
    CalcCplxFIRCoeffs(FIR_Coef_I, FIR_Coef_Q, m_NumTaps, static_cast<float>(-bands.bands[ConfigData.currentBand].FAMCut),
                      static_cast<float>(bands.bands[ConfigData.currentBand].FAMCut), static_cast<float>(SR[static_cast<size_t>(SampleRate)].rate / DF));
  }
  InitFilterMask();
...
void SetDecIntFilters() {
  ...
  ****************************************************************************************/
  ...
  CalcFIRCoeffs(FIR_dec1_coeffs, n_dec1_taps, (float32_t)(LP_F_help), n_att, 0, 0.0, (float32_t)(SR[static_cast<size_t>(SampleRate)].rate));
  CalcFIRCoeffs(FIR_dec2_coeffs, n_dec2_taps, (float32_t)(LP_F_help), n_att, 0, 0.0, (float32_t)(SR[static_cast<size_t>(SampleRate)].rate / DF1));

  CalcFIRCoeffs(FIR_int1_coeffs, 48, (float32_t)(LP_F_help), n_att, 0, 0.0, (float32_t)(SR[static_cast<size_t>(SampleRate)].rate / DF1));
  CalcFIRCoeffs(FIR_int2_coeffs, 32, (float32_t)(LP_F_help), n_att, 0, 0.0, (float32_t)SR[static_cast<size_t>(SampleRate)].rate);
  bin_BW = 1.0 / (DF * FFT_length) * (float32_t)SR[static_cast<size_t>(SampleRate)].rate;
}
