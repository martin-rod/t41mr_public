...
"leaky integrator," as described at:

  https://www.edn.com/a-simple-software-lowpass-filter-suits-embedded-system-applications/

Filter bandwidth is dependent on the sample rate and the "k" parameter, as
follows:
...
Thus, the default values below create a filter with 10000 * 0.0217 = 217 Hz
bandwidth
*/
...
/*****
  ...
  Return value;
    int                   -1 if not valid push button, index of push button if
valid
*****/
...
int Button::ReadSelectedPushButton() {
  ...
  if (buttonInterruptsEnabled) {
    ...
    /*
    Clear the button read.  If the button remains pressed, the ISR will reset
    the value nearly instantly.  Clearing the value here rather than in the ISR
    provides more consistent button press "feel" when calls to
    ReadSelectedPushButton have variable timing.
    */
...
/*****
  Purpose: Function is designed to route program control to the proper execution
point in response to a button press.
...
/*****
  Purpose: Set the radio to a band using the band parameter.  This function is
probably obsolete thanks to the new modal states.
...
/*****
  Purpose: To process a filter button push.  This switches the Filter Encoder
from the lower limit to the upper limit of the audio filter.
...
void Button::ButtonSelectSideband() {
  ...
  case Sideband::BOTH_SAM:
    //  If already in AM or SAM mode, don't need to alter sidebands.  Demod
    //  selection already setup sidebands.
    return;
    break;
  default:
    break;
...
/*****
  Purpose: Mute speaker, headphone, or both with button pushes.  Greg KF5N
February 12, 2025.
...
#define show_FEHelp
  ...
  // Arrays for allocating values associated with keys and switches - choose
  // whether USB keypad or analogue switch matrix USB keypad and analogue switch
  // matrix
