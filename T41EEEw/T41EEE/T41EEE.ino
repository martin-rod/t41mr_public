...
const char *calFilename =
    "/calibration.txt"; // <- SD library uses 8.3 filenames
...
const char *topMenus[] = {
    "CW Options", "RF Set",    "VFO Select",       "Config EEPROM",
    "Cal EEPROM", "AGC",       "Spectrum Options", "SSB Options",
    "EQ Tx Set", // Noise floor removed.  Greg KF5N February 14, 2025
    "EQ Rec Set", "Calibrate", "Bearing"};
// Pointers to functions which execute the menu options.  Do these functions
// used the returned integer???
void (*functionPtr[])() = {
    &CWOptions, &RFOptions, &VFOSelect, &ConfigDataOptions, &CalDataOptions,
    &AGCOptions, &SpectrumOptions,
    //                            button.ButtonMuteAudio, &SSBOptions,
    &SSBOptions, &EqualizerXmtOptions, &EqualizerRecOptions, &CalibrateOptions,
    &BearingMaps};
...
Rotary fineTuneEncoder =
    Rotary(FINETUNE_ENCODER_A, FINETUNE_ENCODER_B); //( 4,  5)
...
ILI9488_t3 tft =
    ILI9488_t3(&SPI, TFT_CS, TFT_DC, TFT_RST); // Instantiate the display.
#else
...
arm_biquad_cascade_df2T_instance_f32 s1_Receive = {
    3, HP_DC_Butter_state, HP_DC_Filter_Coeffs}; // AFP 09-23-22
arm_biquad_cascade_df2T_instance_f32 s1_Receive2 = {
    1, HP_DC_Butter_state2, HP_DC_Filter_Coeffs2}; // AFP 11-04-22
...
Bands bands = {
    {// Revised band struct with mode and sideband.  Greg KF5N February 14, 2025
//                  band low   band hi   name          mode sideband FHiCut
//                  FLoCut FAMCut  Gain  type gain  AGC
// DB2OO, 29-AUG-23: take ITU_REGION into account for band limits.
#if defined(ITU_REGION) && ITU_REGION == 1
     {3700000UL, 3500000, 3800000, "80M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {7150000, 7000000, 7200000, "40M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
#elif defined(ITU_REGION) && ITU_REGION == 2
     {3700000UL, 3500000, 4000000, "80M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {7150000, 7000000, 7300000, "40M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
#elif defined(ITU_REGION) && ITU_REGION == 3
     {3700000UL, 3500000, 3900000, "80M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {7150000, 7000000, 7200000, "40M", RadioMode::SSB_MODE, Sideband::LOWER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
#endif
     {14200000, 14000000, 14350000, "20M", RadioMode::SSB_MODE, Sideband::UPPER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {18100000, 18068000, 18168000, "17M", RadioMode::SSB_MODE, Sideband::UPPER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {21200000, 21000000, 21450000, "15M", RadioMode::SSB_MODE, Sideband::UPPER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {24920000, 24890000, 24990000, "12M", RadioMode::SSB_MODE, Sideband::UPPER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20},
     {28350000, 28000000, 29700000, "10M", RadioMode::SSB_MODE, Sideband::UPPER,
      3000, 200, 5000, 15, HAM_BAND, 1.0, 20}}};
...
dispSc displayScale[] = // dbText, dBScale, baseOffset
    {
        {"20 dB/", 10.0, 24},
        {"10 dB/", 20.0,
         10} //  1, 2, and 5 dB options removed.  Greg KF5N July 30, 2024.
};
...
float32_t *cosBuffer =
    new float32_t[256]; // Was cosBuffer2; Greg KF5N February 7, 2024
float32_t *sinBuffer =
    new float32_t[256]; // This can't be DMAMEM.  It will cause problems with
                        // the CW decoder.
...
float32_t pixel_per_khz = ((1 << ConfigData.spectrum_zoom) * SPECTRUM_RES *
                           1000.0 / SR[SampleRate].rate);
int pos_left = centerLine - (int)(bands.bands[ConfigData.currentBand].FLoCut /
...
const uint32_t N_B =
    FFT_LENGTH / 2 / BUFFER_SIZE * (uint32_t)DF; // 512/2/128 * 8 = 16
...
const float32_t n_fstop2 =
    ((n_samplerate / (DF1 * DF2)) - n_desired_BW) / (n_samplerate / DF1);
...
const uint16_t n_dec1_taps =
    (1 + (uint16_t)(n_att / (22.0 * (n_fstop1 - n_fpass1))));
const uint16_t n_dec2_taps =
    (1 + (uint16_t)(n_att / (22.0 * (n_fstop2 - n_fpass2))));

int attenuator = 0;

int audioYPixel[256]{
    0}; // Will int16_t save memory here???  DMAMEM not working here.  Causes
        // audio spectrum glitch.  KF5N February 26, 2024.
...
int bandswitchPins[] = {
    ...
    29, // 17M
    29, // 15M
    0, // 12M  Note that 12M and 10M both use the 10M filter, which is always in
...
uint32_t s_hotTemp;    /*!< The value of TEMPMON_TEMPSENSE0[TEMP_VALUE] at room
                          temperature .*/
uint32_t s_hotCount; /*!< The value of TEMPMON_TEMPSENSE0[TEMP_VALUE] at the hot
                        temperature.*/
uint32_t currentFreq;
long int n_clear;
uint32_t
    TxRxFreq; // = ConfigData.centerFreq + NCOFreq  NCOFreq from FreqShift2()
...
float32_t biquad_lowpass1_coeffs[5 * N_stages_biquad_lowpass1] = {0, 0, 0, 0,
                                                                  0};
...
float32_t DMAMEM
    FIR_dec1_I_state[n_dec1_taps + (uint16_t)BUFFER_SIZE * (uint32_t)N_B - 1];
...
float32_t DMAMEM
    FIR_dec1_Q_state[n_dec1_taps + (uint16_t)BUFFER_SIZE * (uint16_t)N_B - 1];
...
const float32_t volumeLog[] = {
    0.000010, 0.000011, 0.000013, 0.000014, 0.000016, 0.000018, 0.000020,
    0.000022, 0.000025, 0.000028, 0.000032, 0.000035, 0.000040, 0.000045,
    0.000050, 0.000056, 0.000063, 0.000071, 0.000079, 0.000089, 0.000100,
    0.000112, 0.000126, 0.000141, 0.000158, 0.000178, 0.000200, 0.000224,
    0.000251, 0.000282, 0.000316, 0.000355, 0.000398, 0.000447, 0.000501,
    0.000562, 0.000631, 0.000708, 0.000794, 0.000891, 0.001000, 0.001122,
    0.001259, 0.001413, 0.001585, 0.001778, 0.001995, 0.002239, 0.002512,
    0.002818, 0.003162, 0.003548, 0.003981, 0.004467, 0.005012, 0.005623,
    0.006310, 0.007079, 0.007943, 0.008913, 0.010000, 0.011220, 0.012589,
    0.014125, 0.015849, 0.017783, 0.019953, 0.022387, 0.025119, 0.028184,
    0.031623, 0.035481, 0.039811, 0.044668, 0.050119, 0.056234, 0.063096,
    0.070795, 0.079433, 0.089125, 0.100000, 0.112202, 0.125893, 0.141254,
    0.158489, 0.177828, 0.199526, 0.223872, 0.251189, 0.281838, 0.316228,
    0.354813, 0.398107, 0.446684, 0.501187, 0.562341, 0.630957, 0.707946,
...
/*****
  ...
  Return value;
    void
*****/
void FLASHMEM initTempMon(uint16_t freq, uint32_t lowAlarmTemp,
                          uint32_t highAlarmTemp, uint32_t panicAlarmTemp) {
...
float TGetTemp() {
  ...
  tmeas = s_hotTemp - (float)((nmeas - s_hotCount) * s_hotT_ROOM /
                              s_roomC_hotC); // Calculate temperature
  return tmeas;
}
...
// DB2OO, 11-SEP-23: don't use the fixed sizes, but use the caculated ones,
// otherwise a code change will create very difficult to find problems
#define CLEAR_VAR(x) memset(x, 0, sizeof(x))
  memset(FFT_spec_old, 0, sizeof(FFT_spec_old));
#ifdef DEBUG
  Serial.printf("InitializeDataArrays(): sizeof(FFT_spec_old) %d",
                sizeof(FFT_spec_old));
  Serial.printf("\tsizeof(NR_output_audio_buffer) %d",
                sizeof(NR_output_audio_buffer));
  Serial.println();
#endif
  //  CLEAR_VAR(FFT_spec_old);             //memset(FFT_spec_old, 0, 4096); //
  //  SPECTRUM_RES = 512 * 4 = 2048 CLEAR_VAR(pixelnew); //memset(pixelnew, 0,
  //  1024);                // 512 * 2 CLEAR_VAR(pixelold); //memset(pixelold,
  //  0, 1024);                // 512 * 2 CLEAR_VAR(pixelCurrent);
  ...
  CLEAR_VAR(NR_last_sample_buffer_L); // memset(NR_last_sample_buffer_L, 0,
                                      // 512);
  ...
  //  (float32_t)bands.bands[ConfigData.currentBand].FHiCut,
  //  (float)SR[SampleRate].rate / DF);
  ...
  ****************************************************************************************/
  ...
  biquad_lowpass1.pCoeffs =
      biquad_lowpass1_coeffs; // set pointer to coefficients file

  for (unsigned i = 0; i < 4 * N_stages_biquad_lowpass1; i++) {
    biquad_lowpass1_state[i] = 0.0; // set state variables to zero
  }
  biquad_lowpass1.pState =
      biquad_lowpass1_state; // set pointer to the state variables
  ...
  ****************************************************************************************/
  ...
  //    CalcFIRCoeffs(FIR_dec1_coeffs, 25, (float32_t)5100.0, 80, 0, 0.0,
  //    (float32_t)SR[SampleRate].rate);
  CalcFIRCoeffs(FIR_dec1_coeffs, n_dec1_taps,
                (float32_t)(n_desired_BW * 1000.0), n_att, 0, 0.0,
                (float32_t)SR[SampleRate].rate);

  if (arm_fir_decimate_init_f32(&FIR_dec1_I, n_dec1_taps, (uint32_t)DF1,
                                FIR_dec1_coeffs, FIR_dec1_I_state,
                                BUFFER_SIZE * N_BLOCKS)) {
    while (1)
      ;
  }

  if (arm_fir_decimate_init_f32(&FIR_dec1_Q, n_dec1_taps, (uint32_t)DF1,
                                FIR_dec1_coeffs, FIR_dec1_Q_state,
                                BUFFER_SIZE * N_BLOCKS)) {
    while (1)
      ;
  }
  // Decimation filter 2, M2 = DF2
  CalcFIRCoeffs(FIR_dec2_coeffs, n_dec2_taps,
                (float32_t)(n_desired_BW * 1000.0), n_att, 0, 0.0,
                (float32_t)(SR[SampleRate].rate / DF1));
  if (arm_fir_decimate_init_f32(&FIR_dec2_I, n_dec2_taps, (uint32_t)DF2,
                                FIR_dec2_coeffs, FIR_dec2_I_state,
                                BUFFER_SIZE * N_BLOCKS / (uint32_t)DF1)) {
    while (1)
      ;
  }

  if (arm_fir_decimate_init_f32(&FIR_dec2_Q, n_dec2_taps, (uint32_t)DF2,
                                FIR_dec2_coeffs, FIR_dec2_Q_state,
                                BUFFER_SIZE * N_BLOCKS / (uint32_t)DF1)) {
    while (1)
      ;
  }
  ...
  // yes, because the interpolation filter is AFTER the upsampling, so it has to
  // be in the target sample rate!
  CalcFIRCoeffs(FIR_int1_coeffs, 48, (float32_t)(n_desired_BW * 1000.0), n_att,
                0, 0.0, SR[SampleRate].rate / 4.0);
  if (arm_fir_interpolate_init_f32(&FIR_int1_I, (uint8_t)DF2, 48,
                                   FIR_int1_coeffs, FIR_int1_I_state,
                                   BUFFER_SIZE * N_BLOCKS / (uint32_t)DF)) {
    while (1)
      ;
  }
  if (arm_fir_interpolate_init_f32(&FIR_int1_Q, (uint8_t)DF2, 48,
                                   FIR_int1_coeffs, FIR_int1_Q_state,
                                   BUFFER_SIZE * N_BLOCKS / (uint32_t)DF)) {
    while (1)
      ;
  }
  ...
  // yes, because the interpolation filter is AFTER the upsampling, so it has to
  // be in the target sample rate!
  CalcFIRCoeffs(FIR_int2_coeffs, 32, (float32_t)(n_desired_BW * 1000.0), n_att,
                0, 0.0, (float32_t)SR[SampleRate].rate);

  if (arm_fir_interpolate_init_f32(&FIR_int2_I, (uint8_t)DF1, 32,
                                   FIR_int2_coeffs, FIR_int2_I_state,
                                   BUFFER_SIZE * N_BLOCKS / (uint32_t)DF1)) {
    while (1)
      ;
  }
  if (arm_fir_interpolate_init_f32(&FIR_int2_Q, (uint8_t)DF1, 32,
                                   FIR_int2_coeffs, FIR_int2_Q_state,
                                   BUFFER_SIZE * N_BLOCKS / (uint32_t)DF1)) {
    while (1)
      ;
  }

  SetDecIntFilters(); // The correct bandwidths are calculated and set
                      // accordingly.
  ZoomFFTPrep();
  ...
  initTempMon(temp_check_frequency, lowAlarmTemp, highAlarmTemp,
              panicAlarmTemp);
...
FLASHMEM void Splash() {
  ...
  tft.setCursor((XPIXELS / 2) - (12 * tft.getFontWidth()) / 2,
                YPIXELS / 2 + 110); // 12 = letters in "Property of:"
...
/*****
  Purpose: Program entry point for configuration prior to running the main
loop() function.
...
FLASHMEM void setup() {
  ...
  setSyncProvider(
      getTeensy3Time); // get TIME from real time clock with 3V backup battery
  setTime(now());
  ...
  sgtl5000_1.enable();
  sgtl5000_1.setAddress(LOW); // This sets to one of two possible I2C addresses,
                              // controlled by a jumper on the Audio Adapter.
  //  Don't use the audio pre-processor.  This causes a spurious signal in the
  //  SSB transmit output. sgtl5000_1.audioPreProcessorEnable();  // Need to use
  //  one of the equalizers.
  AudioMemory(250);
  AudioMemory_F32(100);
  ...
  sgtl5000_1.lineOutLevel(
      13); // Setting of 13 limits line-out level to 3.15 volts p-p (maximum).
#else
  sgtl5000_1.lineOutLevel(
      20); // Setting of 20 limits line-out level to 2.14 volts p-p.
#endif
  sgtl5000_1
      .adcHighPassFilterEnable(); // This is required for QSE2DC, specifically
                                  // for carrier calibration.
                                  //  sgtl5000_1.adcHighPassFilterDisable();
                                  //  //reduces noise.
                                  //  https://forum.pjrc.com/threads/27215-24-bit-audio-boards?p=78831&viewfull=1#post78831

  updateMic(); // This updates the transmit signal chain settings.  Located in
               // SSB_Exciter.cpp.
  ...
  cessb1.setProcessing(
      ConfigData.cessb); // Set to CESSB or SSB Data.  Greg KF5N August 17 2024

  Q_out_L_Ex.setMaxBuffers(
      32); // Limits determined emperically.  These may need more adjustment.
  ...
  pinMode(MUTE, OUTPUT);
  digitalWrite(MUTE,
               MUTEAUDIO); // Keep audio junk out of the speakers/headphones
                           // until configuration is complete.
  pinMode(PTT, INPUT_PULLUP);
  pinMode(BUSY_ANALOG_PIN,
          INPUT); // Pin 39.  Switch matrix output connects to this pin.
  ...
  pinMode(TFT_CS,
          OUTPUT); // The Main board has a pull-up resistor on an output???
  digitalWrite(TFT_CS, HIGH);

  // Rotary encoders.
  tuneEncoder.begin(true);
  volumeEncoder.begin(true);
  attachInterrupt(digitalPinToInterrupt(VOLUME_ENCODER_A), EncoderVolume,
                  CHANGE);
  attachInterrupt(digitalPinToInterrupt(VOLUME_ENCODER_B), EncoderVolume,
                  CHANGE);
  filterEncoder.begin(true);
  attachInterrupt(digitalPinToInterrupt(FILTER_ENCODER_A), EncoderFilter,
                  CHANGE);
  attachInterrupt(digitalPinToInterrupt(FILTER_ENCODER_B), EncoderFilter,
                  CHANGE);
  fineTuneEncoder.begin(true);
  attachInterrupt(digitalPinToInterrupt(FINETUNE_ENCODER_A), EncoderFineTune,
                  CHANGE);
  attachInterrupt(digitalPinToInterrupt(FINETUNE_ENCODER_B), EncoderFineTune,
                  CHANGE);
  attachInterrupt(
      digitalPinToInterrupt(KEYER_DIT_INPUT_TIP), KeyTipOn,
      CHANGE); // Changed to keyTipOn from KeyOn everywhere JJP 8/31/22
  attachInterrupt(digitalPinToInterrupt(KEYER_DAH_INPUT_RING), KeyRingOn,
                  CHANGE);

  // Move these to the initialization function?
  arm_fir_init_f32(&FIR_CW_DecodeL, 64, CW_Filter_Coeffs2, FIR_CW_DecodeL_state,
                   256); // AFP 10-25-22
  arm_fir_init_f32(&FIR_CW_DecodeR, 64, CW_Filter_Coeffs2, FIR_CW_DecodeR_state,
                   256);
  arm_fir_interpolate_init_f32(&FIR_int1_EX_I, 2, 48, coeffs48K_8K_LPF_FIR,
                               FIR_int1_EX_I_state, 256); // Used in CW exciter.
  arm_fir_interpolate_init_f32(&FIR_int1_EX_Q, 2, 48, coeffs48K_8K_LPF_FIR,
                               FIR_int1_EX_Q_state, 256);
  arm_fir_interpolate_init_f32(&FIR_int2_EX_I, 4, 32, coeffs192K_10K_LPF_FIR,
                               FIR_int2_EX_I_state, 512);
  arm_fir_interpolate_init_f32(&FIR_int2_EX_Q, 4, 32, coeffs192K_10K_LPF_FIR,
                               FIR_int2_EX_Q_state, 512);
  ...
  *(digital_pin_to_info_PGM + TFT_CS)->pad = iospeed_display;
  ...
  tft.begin(RA8875_800x480, 8, 20000000UL,
            4000000UL); // parameter list from library code
  tft.setRotation(0);

  // Setup for scrolling attributes. Part of initSpectrum_RA8875() call written
  // by Mike Lewis
  ...
  ConfigData.sdCardPresent =
      InitializeSDCard(); // Initialize mandatory SD card.
  ...
  ****************************************************************************************/
  ...
  si5351.init(SI5351_CRYSTAL_LOAD_10PF, Si_5351_crystal,
              CalData.freqCorrectionFactor); // JJP  7/14/23
  si5351.set_ms_source(
      SI5351_CLK2,
      SI5351_PLLB); // Allows CLK1 and CLK2 to exceed 100 MHz simultaneously.
#ifdef PLLMODULE
  ...
  lastState =
      RadioState::NOSTATE; // To make sure the receiver will be configured on
  ...
  mainMenuIndex = 0; // Changed from middle to first. Do Menu Down to get to
                     // Calibrate quickly
  zoomIndex =
      ConfigData.spectrum_zoom -
      1; // ButtonZoom() increments zoomIndex, so this cancels it so the read
         // from EEPROM is accurately restored.  KF5N August 3, 2023
...
elapsedMicros usec =
    0; // Automatically increases as time passes; no ++ necessary.
...
void loop() {
  ...
  menu = readButton();
  if (menu != MenuSelect::BOGUS_PIN_READ and
      (radioState != RadioState::SSB_TRANSMIT_STATE) and
      (radioState != RadioState::FT8_TRANSMIT_STATE) and
      (radioState != RadioState::CW_TRANSMIT_STRAIGHT_STATE) and
  ...
  if (lastState != radioState) {
    ...
    //    Serial.printf("Set audio state, begin loop. radioState = %d lastState
    //    = %d\n", radioState, lastState);
  }

  // Don't turn off audio in the case of CW for sidetone.
  if (powerUp and not(radioState == RadioState::CW_TRANSMIT_STRAIGHT_STATE or
                      radioState == RadioState::CW_TRANSMIT_KEYER_STATE)) {
  ...
  if (radioState == RadioState::CW_TRANSMIT_STRAIGHT_STATE or
      radioState == RadioState::CW_TRANSMIT_KEYER_STATE) {
    speakerScale.setGain(SPEAKERSCALE);
    headphoneScale.setGain(HEADPHONESCALE);
  }
  ...
  case RadioState::SSB_TRANSMIT_STATE:
    ...
    while (digitalRead(PTT) == LOW) {
      ...
      if (cessb1.levelDataCount() > 2000) {
        Serial.print("levelDataCount = "); // Before getLevels(1), because it
                                           // resets counts to 0.
        Serial.println(cessb1.levelDataCount());
        cessbData =
            cessb1.getLevels(1); // Update the CESSB information struct.  Write
        ...
        float32_t enhance = (10.0f * log10f(cessbData->pwr1) -
                             20.0f * log10f(cessbData->peak1)) -
                            (10.0f * log10f(cessbData->pwr0) -
                             20.0f * log10f(cessbData->peak0));
  ...
  case RadioState::CW_TRANSMIT_STRAIGHT_STATE:
    ...
    cwTimer = millis();
    while (millis() - cwTimer <=
           ConfigData.cwTransmitDelay) { // Start CW transmit timer on
      digitalWrite(RXTX, HIGH);
      if (digitalRead(ConfigData.paddleDit) == LOW &&
          ConfigData.keyType == 0) { // AFP 09-25-22  Turn on CW signal
        ...
        if (digitalRead(ConfigData.paddleDit) == HIGH &&
            ConfigData.keyType == 0) { // Turn off CW signal
          keyPressedOn = 0;
  ...
  case RadioState::CW_TRANSMIT_KEYER_STATE:
    ...
    while (millis() - cwTimer <= ConfigData.cwTransmitDelay) {
      ...
      if (digitalRead(ConfigData.paddleDit) == LOW) { // Keyer Dit
        ...
        CW_ExciterIQData(CW_SHAPING_RISE);
        for (cwBlockIndex = 0; cwBlockIndex < transmitDitUnshapedBlocks;
             cwBlockIndex++) {
          CW_ExciterIQData(CW_SHAPING_NONE);
        }
        CW_ExciterIQData(CW_SHAPING_FALL);
        ...
        CW_ExciterIQData(CW_SHAPING_RISE);
        for (cwBlockIndex = 0; cwBlockIndex < transmitDahUnshapedBlocks;
             cwBlockIndex++) {
          CW_ExciterIQData(CW_SHAPING_NONE);
        }
        CW_ExciterIQData(CW_SHAPING_FALL);
